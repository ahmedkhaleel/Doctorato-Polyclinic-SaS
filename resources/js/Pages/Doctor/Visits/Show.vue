...
const vitalMetrics = computed(() => [
    { key: 'bp', label: isRtl.value ? 'ضغط الدم' : 'Blood Pressure', icon: '🫀', color: '#EF4444', unit: 'mmHg' },
    { key: 'hr', label: isRtl.value ? 'النبض' : 'Heart Rate', icon: '💓', color: '#EC4899', unit: 'bpm' },
    { key: 'temp', label: isRtl.value ? 'الحرارة' : 'Temperature', icon: '🌡️', color: '#F59E0B', unit: '°C' },
    { key: 'spo2', label: 'SpO2', icon: '🫁', color: '#3B82F6', unit: '%' },
    { key: 'sugar', label: isRtl.value ? 'السكر' : 'Blood Sugar', icon: '🩸', color: '#8B5CF6', unit: 'mg/dL' },
    { key: 'weight', label: isRtl.value ? 'الوزن' : 'Weight', icon: '⚖️', color: '#10B981', unit: 'kg' },
]);

const historyData = computed(() => props.vitalsHistory || []);
const hasHistory = computed(() => historyData.value.length > 1);

// Get chart data points for selected metric
...
// Get change indicator between current and previous reading
    ...
    <div class="space-y-6">
                ...
                >
                    <div class="flex items-center justify-between px-6 py-4 border-b border-gray-100">
                        <div class="flex items-center gap-2">
                            ...
                            <span v-if="visit.prescriptions?.length" class="text-[10px] font-bold text-rose-600 bg-rose-50 px-1.5 py-0.5 rounded-full border border-rose-100">{{ visit.prescriptions.length }}</span>
                        </div>
                    ...
                    <div class="p-6">
                                ...
                                <form @submit.prevent="submitPrescription" class="space-y-4">
                                    <div class="grid lg:grid-cols-2 gap-3">
                                        <div>
                                            ...
                                            <input v-model="prescriptionForm.diagnosis" type="text" class="w-full px-3 py-2.5 border border-gray-200 rounded-xl text-sm focus:ring-2 focus:ring-[#C4A265]/30 focus:border-[#C4A265]" :placeholder="isRtl ? 'التشخيص...' : 'Diagnosis...'" />
                                        </div>
                                        <div>
                                            <label class="text-xs text-gray-500 font-medium mb-1 block">{{ isRtl ? 'ملاحظات' : 'Notes' }}</label>
                                            <input v-model="prescriptionForm.notes" type="text" class="w-full px-3 py-2.5 border border-gray-200 rounded-xl text-sm focus:ring-2 focus:ring-[#C4A265]/30 focus:border-[#C4A265]" :placeholder="isRtl ? 'ملاحظات...' : 'Notes...'" />
                                        </div>
                                    </div>
                                    <div>
                                        <div class="flex items-center justify-between mb-3">
                        ...
                        <div v-if="visit.prescriptions?.length > 0" class="space-y-3">
                            <div v-for="prescription in visit.prescriptions" :key="prescription.id" class="border border-gray-100 rounded-xl p-4 hover:border-gray-200 transition-colors">
                                <div class="flex items-center justify-between mb-3">
                                    <div class="flex items-center gap-2">
                                        <div class="w-6 h-6 rounded-full bg-rose-50 flex items-center justify-center">
                                            <svg class="w-3 h-3 text-rose-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" /></svg>
                                        </div>
                                    ...
                                    <table class="min-w-full text-xs">
                                        ...
                                        <tbody class="divide-y divide-gray-50">
                                            ...
                                            >
                                                ...
                                                <td class="px-3 py-2.5 text-center">
                                                    <span v-if="reading.temp" :class="reading.temp > 38 ? 'text-red-600 font-semibold' : 'text-gray-700'">{{ reading.temp }}°</span>
                                                    <span v-else class="text-gray-300">—</span>
                                                </td>
                                                <td class="px-3 py-2.5 text-center">
        ...
        <Teleport to="body">
            <Transition enter-active-class="transition-all duration-200" enter-from-class="opacity-0" enter-to-class="opacity-100" leave-active-class="transition-all duration-150" leave-from-class="opacity-100" leave-to-class="opacity-0">
                <div v-if="showConfirmComplete" class="fixed inset-0 z-[9999] flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm" @click.self="showConfirmComplete = false">
        ...
        <Teleport to="body">
            <Transition enter-active-class="transition-all duration-200" enter-from-class="opacity-0" enter-to-class="opacity-100" leave-active-class="transition-all duration-150" leave-from-class="opacity-100" leave-to-class="opacity-0">
                <div v-if="showConfirmCancel" class="fixed inset-0 z-[9999] flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm" @click.self="showConfirmCancel = false">
                    <Transition enter-active-class="transition-all duration-200" enter-from-class="opacity-0 scale-95" enter-to-class="opacity-100 scale-100" leave-active-class="transition-all duration-150" leave-from-class="opacity-100 scale-100" leave-to-class="opacity-0 scale-95" appear>
                        <div class="bg-white rounded-2xl shadow-2xl w-full max-w-md p-6">
                            <div class="flex items-center gap-3 mb-4">
