...
watch(search, (val) => {
    ...
    searchTimeout = setTimeout(() => {
        router.get('/webmaster/post-categories', { search: val || undefined }, {
            preserveState: true,
            replace: true,
        });
    }, 400);
...
<template>
    <WebmasterLayout title="Post Categories">
        <div class="space-y-6">
            <div class="flex items-center justify-between">
                <h1 class="text-2xl font-bold text-gray-800">{{ $t('a_post_categories') }}</h1>
                <Link
                    v-if="can('post_categories.create')"
                    href="/webmaster/post-categories/create"
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            <tr v-for="category in categories.data" :key="category.id" class="hover:bg-gray-50">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap ltr:text-right rtl:text-left text-sm ltr:space-x-3 rtl:space-x-reverse rtl:space-x-3">
                                    <Link v-if="can('post_categories.update')" :href="`/webmaster/post-categories/${category.id}/edit`" class="font-medium hover:underline" style="color: #C4A265;">{{ $t('a_edit') }}</Link>
                            ...
                            <tr v-if="!categories.data || categories.data.length === 0">
                                <td colspan="5" class="px-6 py-8 text-center text-sm text-gray-500">{{ $t('a_no_post_categories_found') }}</td>
                            </tr>
                        </tbody>
                    </table>
                </div>
