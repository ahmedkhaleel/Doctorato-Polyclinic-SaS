...
const form = useForm({
    name: '', description: '', lead_source_id: '', status: 'draft',
    budget: '', actual_cost: '', start_date: '', end_date: '',
    utm_source: '', utm_medium: '', utm_campaign: '',
});

function submit() { form.post('/admin/campaigns'); }
...
function getSelectedSourceName() {
    if (!form.lead_source_id || !props.sources) return '';
    const s = props.sources.find(src => src.id === form.lead_source_id);
    return s ? s.name_en : '';
}
</script>

<template>
    <AdminLayout :title="$t('a_new_campaign')">
        <form @submit.prevent="submit" class="space-y-6 pb-24">
            ...
            >
                <div class="flex items-center gap-4">
                    <Link href="/admin/campaigns" class="inline-flex items-center justify-center w-10 h-10 rounded-xl bg-white border border-gray-200 text-gray-500 hover:text-gray-700 hover:border-gray-300 hover:shadow-md hover:-translate-y-0.5 transition-all duration-200">
                    ...
                    <div class="flex items-center gap-3">
                        ...
                        <div>
                            <h1 class="text-xl md:text-2xl font-bold text-gray-900 tracking-tight">{{ $t('a_new_campaign') }}</h1>
                            <p class="text-sm text-gray-500 mt-0.5">{{ $t('a_new_campaign_description') }}</p>
                        </div>
                    </div>
                </div>
            </div>
                        ...
                        <div>
                            ...
                            <div class="flex gap-3">
                                <button
                                    ...
                                    class="flex-1 flex items-center justify-center gap-2 px-4 py-3 text-sm font-medium border rounded-xl transition-all duration-200"
                                >
                ...
                <div class="bg-white/95 backdrop-blur-sm border-t border-gray-200 shadow-[0_-4px_20px_rgba(0,0,0,0.06)]">
                    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 flex items-center justify-between">
                        <div class="flex items-center gap-3">
                            ...
                            <span class="text-sm text-gray-500">{{ $t('a_unsaved_changes') }}</span>
                        </div>
                        <div class="flex items-center gap-3">
                            <Link href="/admin/campaigns" class="px-5 py-2.5 text-sm font-medium text-gray-600 hover:text-gray-800 hover:bg-gray-100 rounded-xl transition-all duration-200">{{ $t('a_cancel') }}</Link>
                            <button
                                type="submit"
                            ...
                            >
                                <span v-if="form.processing" class="flex items-center gap-2">
                                    <svg class="animate-spin w-4 h-4" fill="none" viewBox="0 0 24 24"><circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle><path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z"></path></svg>{{ $t('a_creating') }}</span>
                                <span v-else class="flex items-center gap-2">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7" /></svg>{{ $t('a_create_campaign') }}</span>
                            </button>
                        </div>
                    </div>
                </div>
