...
const typeConfig = {
    ...
    usage:            { label: 'Usage',            bg: 'bg-slate-50',    text: 'text-[#1B365D]',    dot: 'bg-[#1B365D]',    sign: '-' },
    adjustment:       { label: 'Adjustment',       bg: 'bg-amber-50',   text: 'text-amber-700',   dot: 'bg-amber-500',   sign: '' },
    return:           { label: 'Return',           bg: 'bg-slate-50',  text: 'text-[#1B365D]',  dot: 'bg-[#1B365D]',  sign: '+' },
    ...
    <div class="space-y-6">
        ...
        >
            ...
            <div class="relative z-10">
                ...
                <Link
                    href="/admin/supplies"
                    class="inline-flex items-center gap-1.5 text-sm text-slate-200/70 hover:text-white transition-colors mb-4"
                >
                ...
                <div class="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
                    <div class="flex-1 min-w-0">
                        ...
                        <div class="flex items-center gap-3 flex-wrap mb-2">
                            <h1 class="text-xl md:text-2xl sm:text-3xl font-bold text-white truncate">
                                {{ displayName(supply) }}
                            </h1>
                            <span
                        ...
                        <!-- Secondary info -->
                        <div class="flex items-center gap-4 flex-wrap text-sm text-slate-200/70">
                            ...
                            <span
                                ...
                                :class="supply.module === 'dental' ? 'bg-slate-400/20 text-slate-300' : 'bg-amber-400/20 text-amber-300'"
                            >
                                {{ supply.module }}
                            </span>
                ...
                <div class="mt-6 grid grid-cols-2 sm:grid-cols-4 gap-3">
                    <div class="bg-white/10 backdrop-blur-sm rounded-xl px-4 py-3">
                        <p class="text-xs text-slate-200/60">Current Stock</p>
                        <p class="text-xl font-bold text-white">
                            {{ supply.quantity }}
                            <span class="text-sm font-normal text-slate-200/50">{{ supply.unit || '' }}</span>
                        </p>
                    </div>
                    <div class="bg-white/10 backdrop-blur-sm rounded-xl px-4 py-3">
                        <p class="text-xs text-slate-200/60">Min Quantity</p>
                        <p class="text-xl font-bold text-white">{{ supply.min_quantity || 0 }}</p>
                    </div>
                    <div class="bg-white/10 backdrop-blur-sm rounded-xl px-4 py-3">
                        <p class="text-xs text-slate-200/60">Purchase Price</p>
                        <p class="text-xl font-bold text-white">{{ formatCurrency(supply.purchase_price) }}</p>
                    </div>
                    <div class="bg-white/10 backdrop-blur-sm rounded-xl px-4 py-3">
                        <p class="text-xs text-slate-200/60">Total Value</p>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div class="lg:col-span-2 space-y-6">
                ...
                <!-- Product Info Card -->
                <div
                    class="bg-white rounded-2xl shadow-sm border border-gray-100 p-4 md:p-6 transition-all duration-700"
                ...
                >
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-x-8 gap-y-5">
                        ...
                        <div>
                            ...
                            <p class="text-sm font-mono text-[#1B365D]">{{ supply.sku || '-' }}</p>
                        </div>
                        <div>
                ...
                >
                    ...
                    <div class="px-4 md:px-6 py-4 border-b border-gray-100 flex items-center justify-between">
                        ...
                        <button
                            ...
                            :class="showTransactionForm
                                ...
                                : 'text-white hover:shadow-lg hover:shadow-[#1B365D]/25'"
                    ...
                    >
                        <div v-if="showTransactionForm" class="overflow-hidden">
                            <form @submit.prevent="submitTransaction" class="p-4 md:p-6 bg-slate-50/50 border-b border-gray-100">
                                ...
                                <div class="mt-4 flex justify-end">
                                    <button
                                        ...
                                        class="inline-flex items-center px-4 md:px-6 py-2.5 rounded-xl text-white text-sm font-medium transition-all disabled:opacity-50 hover:shadow-lg hover:shadow-[#1B365D]/25"
                    ...
                    <div class="p-6">
                        <div v-if="transactions.data && transactions.data.length > 0" class="relative">
                            <!-- Timeline line -->
                            <div :class="isRtl ? 'right-3' : 'left-3'" class="absolute top-2 bottom-2 w-px bg-gradient-to-b from-slate-200 via-gray-200 to-transparent"></div>
                    ...
                    <!-- Pagination -->
                    <div v-if="transactions.links && transactions.links.length > 3" class="px-4 md:px-6 py-3 border-t border-gray-100 flex items-center justify-between">
                    ...
                    <div class="flex justify-center mb-5">
                        <div class="relative w-36 h-36">
                            ...
                            <div class="absolute inset-0 flex flex-col items-center justify-center">
                                <span class="text-xl md:text-2xl font-bold" :style="{ color: gaugeColor }">{{ stockPercentage }}%</span>
                ...
                >
                    ...
                    <div class="space-y-4">
                        <div>
                            ...
                            <p class="text-lg font-bold text-[#1B365D]">{{ formatCurrency(supply.purchase_price) }}</p>
                        </div>
                        <div>
                ...
                >
                    ...
                    <div class="space-y-2">
                        <div
                            ...
                            :key="index"
                            class="flex items-center justify-between py-2.5 px-3 rounded-xl hover:bg-slate-50/50 transition-colors"
                            :class="index > 0 ? 'border-t border-gray-100' : ''"
                        >
                            <div class="flex items-center gap-2">
                                <div class="w-2 h-2 rounded-full flex-shrink-0 bg-[#1B365D]"></div>
                                <span class="text-sm text-gray-800">{{ ss.service?.name_en || 'Unknown Service' }}</span>
                            </div>
                            <span class="text-xs font-semibold px-2.5 py-1 rounded-lg bg-slate-50 text-[#1B365D]">
