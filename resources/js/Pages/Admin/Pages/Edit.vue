    ...
    <AdminLayout :title="$t('a_edit_page')">
        <div class="space-y-6">
            <div class="flex items-center justify-between">
                <h1 class="text-xl md:text-2xl font-bold text-gray-800">{{ $t('a_edit_page') }}: {{ $localized(page, 'title') }}</h1>
            ...
            <form @submit.prevent="submit" class="max-w-5xl space-y-6">
                ...
                <div class="flex space-x-3">
                    ...
                    <Link href="/admin/pages" class="py-2.5 px-4 md:px-6 rounded-lg bg-gray-200 text-gray-700 text-sm font-medium hover:bg-gray-300 transition">
                        {{ $t('a_cancel') }}
                    </Link>
                </div>
            </form>
