    ...
    <AdminLayout :title="$t('a_penalties_rewards')">
        <div class="space-y-6">
            ...
            <div class="flex items-center justify-between">
                <h1 class="text-2xl font-bold text-gray-800">{{ $t('a_penalties_rewards') }}</h1>
                <button
                ...
                <div class="bg-white rounded-lg shadow-sm p-5 ltr:border-l-4 rtl:border-r-4 border-red-400">
                    <div class="flex items-center justify-between">
                        <div>
                            ...
                            <p class="text-2xl font-bold text-red-600">{{ stats?.total_penalties || 0 }}</p>
                        </div>
                ...
                <div class="bg-white rounded-lg shadow-sm p-5 ltr:border-l-4 rtl:border-r-4 border-green-400">
                    <div class="flex items-center justify-between">
                        <div>
                            ...
                            <p class="text-2xl font-bold text-green-600">{{ stats?.total_rewards || 0 }}</p>
                        </div>
                ...
                <div class="bg-white rounded-lg shadow-sm p-5 ltr:border-l-4 rtl:border-r-4 border-red-400">
                    <div class="flex items-center justify-between">
                        <div>
                            ...
                            <p class="text-2xl font-bold text-red-600">{{ formatCurrency(stats?.penalties_amount) }}</p>
                        </div>
                        <div class="w-10 h-10 rounded-full bg-red-100 flex items-center justify-center">
                            <svg class="w-5 h-5 text-red-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <div class="bg-white rounded-lg shadow-sm p-5 ltr:border-l-4 rtl:border-r-4 border-green-400">
                    <div class="flex items-center justify-between">
                        <div>
                            ...
                            <p class="text-2xl font-bold text-green-600">{{ formatCurrency(stats?.rewards_amount) }}</p>
                        </div>
                        <div class="w-10 h-10 rounded-full bg-green-100 flex items-center justify-center">
                            <svg class="w-5 h-5 text-green-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8c1.11 0 2.08.402 2.599 1M12 8V7m0 1v8m0 0v1m0-1c-1.11 0-2.08-.402-2.599-1M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
            ...
            <div class="bg-white rounded-lg shadow-sm overflow-hidden">
                    ...
                    <table class="min-w-full divide-y divide-gray-200">
                        <thead class="bg-gray-50">
                            <tr>
                                ...
                                <th class="px-6 py-3 ltr:text-left rtl:text-right text-xs font-medium text-gray-500 uppercase tracking-wider">{{ $t('a_applied_to_salary') }}</th>
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            <tr v-for="record in penalties.data" :key="record.id" class="hover:bg-gray-50">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full" :class="record.type === 'penalty' ? 'bg-red-100 text-red-700' : 'bg-green-100 text-green-700'">
                                        {{ record.type === 'penalty' ? $t('a_penalty') : $t('a_reward') }}
                                    </span>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm font-semibold" :style="record.type === 'penalty' ? 'color: #dc2626;' : 'color: #16a34a;'">{{ formatCurrency(record.amount) }}</td>
                                <td class="px-6 py-4 text-sm text-gray-500 max-w-[200px] truncate">{{ record.reason || '-' }}</td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ record.date }}</td>
                                <td class="px-6 py-4 whitespace-nowrap text-center">
                                    <svg v-if="record.applied_to_salary" class="w-5 h-5 text-green-500 inline-block" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            ...
                            <tr v-if="!penalties.data || penalties.data.length === 0">
                                <td colspan="7" class="px-6 py-8 text-center text-sm text-gray-500">{{ $t('a_no_records_found') }}</td>
                            </tr>
                        </tbody>
                    </table>
                </div>
                ...
                <!-- Pagination -->
                <div v-if="penalties.links && penalties.links.length > 3" class="px-6 py-3 border-t border-gray-200 flex items-center justify-between">
            ...
            <div v-if="showModal" class="fixed inset-0 z-50 overflow-y-auto">
                <div class="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:p-0">
                    ...
                    <div class="relative bg-white rounded-lg shadow-xl w-full max-w-lg mx-auto z-10">
                        ...
                        <form @submit.prevent="submitRecord" class="px-6 py-4 space-y-4">
                            <div>
                                <label class="block text-sm font-medium text-gray-700 mb-1">{{ $t('a_employee') }} <span class="text-red-500">*</span></label>
                                <select v-model="form.employee_id" class="w-full px-4 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-yellow-200 focus:border-transparent">
                            ...
                            <div>
                                ...
                                <div class="flex gap-6">
                                    ...
                                    <label class="inline-flex items-center cursor-pointer">
                                        <input type="radio" v-model="form.type" value="reward" class="form-radio text-green-600 focus:ring-green-500" />
                            ...
                            <div class="grid grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <input v-model="form.amount" type="number" step="0.01" min="0" class="w-full px-4 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-yellow-200 focus:border-transparent" placeholder="0.00" />
                            ...
                            <div>
                                ...
                                <textarea v-model="form.reason" rows="3" class="w-full px-4 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-yellow-200 focus:border-transparent" :placeholder="$t('a_reason_for_penalty_reward')"></textarea>
                            ...
                            <div :class="['flex pt-2 border-t border-gray-200', isRtl ? 'justify-start space-x-reverse space-x-3' : 'justify-end space-x-3']">
                                ...
                                <button type="submit" :disabled="formProcessing" class="px-6 py-2 rounded-lg text-white text-sm font-medium transition disabled:opacity-50" style="background-color: #C4A265;">{{ formProcessing ? $t('a_saving') : $t('a_create_record') }}</button>
                            </div>
                        </form>
                    </div>
                </div>
