        ...
        <div class="receipt-container mx-auto max-w-[800px] bg-white rounded-2xl shadow-sm border border-gray-100/80 print:shadow-none print:border-0 print:rounded-none">
            <div class="p-4 md:p-8 print:p-0">
                ...
                <div class="text-center mb-8 pb-6 border-b border-gray-200">
                    <img src="/images/logo.png" alt="Doctorato Polyclinic" class="mx-auto h-20 mb-3 object-contain" />
                ...
                <div class="grid grid-cols-2 gap-6 mb-8">
                    <div class="space-y-2">
                        ...
                        <div class="flex items-baseline gap-2">
                            ...
                            <span class="text-sm font-semibold font-mono" style="color: #C4A265;">{{ invoice.invoice_number }}</span>
                        </div>
                        <div class="flex items-baseline gap-2">
                            <span class="text-xs text-gray-500">{{ $t('a_invoice_date') }}:</span>
                    ...
                    <div class="space-y-2">
                        ...
                        <div v-if="invoice.patient?.file_number" class="flex items-baseline gap-2">
                            ...
                            <span class="text-sm font-semibold font-mono" style="color: #C4A265;">{{ invoice.patient.file_number }}</span>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="px-4 md:px-6 py-5">
                        <div class="grid grid-cols-2 gap-4">
                            ...
                            <div v-if="payment.reference_number">
                                <p class="text-xs text-gray-400 mb-1">{{ $t('a_reference_number') }}</p>
                ...
                <!-- Invoice Items Table -->
                <div class="mb-8">
                    <h3 class="text-xs font-semibold text-gray-400 uppercase tracking-wider mb-3">{{ $t('a_invoice_items') }}</h3>
                    <table class="w-full text-sm border border-gray-200">
                        <thead>
                            <tr class="bg-gray-50">
                                <th class="px-4 py-2.5 text-start text-xs font-semibold text-gray-600 border-b border-gray-200">{{ $t('a_description') }}</th>
                                <th class="px-4 py-2.5 text-center text-xs font-semibold text-gray-600 border-b border-gray-200">{{ $t('a_qty') }}</th>
                        ...
                        <tbody class="divide-y divide-gray-100">
                            <tr v-for="(item, index) in invoice.items" :key="index">
                                ...
                                <td class="px-4 py-2.5 text-right text-gray-600">{{ formatCurrency(item.unit_price) }}</td>
                                <td class="px-4 py-2.5 text-right font-semibold text-gray-800">{{ formatCurrency(item.total) }}</td>
                            </tr>
                        </tbody>
                        <tfoot>
                            <tr v-if="invoice.discount_amount > 0" class="border-t border-gray-200">
                                <td colspan="3" class="px-4 py-2 text-end text-sm text-gray-500">{{ $t('a_discount') }}</td>
                                <td class="px-4 py-2 text-right text-sm text-red-600">-{{ formatCurrency(invoice.discount_amount) }}</td>
                            </tr>
                            <tr class="bg-gray-50 border-t-2 border-gray-300">
                                <td colspan="3" class="px-4 py-3 text-end text-sm font-bold text-gray-700">{{ $t('a_invoice_total') }}</td>
                                <td class="px-4 py-3 text-right text-sm font-bold" style="color: #C4A265;">{{ formatCurrency(invoiceTotal) }}</td>
                            </tr>
                        </tfoot>
                    </table>
                </div>
                        ...
                        <table v-if="allPayments && allPayments.length > 0" class="w-full text-sm">
                            <thead>
                                <tr class="bg-gray-50">
                                    ...
                                    <th class="px-4 py-2.5 text-start text-xs font-semibold text-gray-600 border-b border-gray-200">{{ $t('a_reference') }}</th>
                                </tr>
                            </thead>
                            <tbody class="divide-y divide-gray-100">
                                <tr v-for="(p, idx) in allPayments" :key="p.id" :class="p.id === payment.id ? 'bg-amber-50/60' : ''">
                                    <td class="px-4 py-2.5 text-gray-500 text-xs font-medium">
                                        {{ idx + 1 }}
                                        <span v-if="p.id === payment.id" class="ltr:ml-1 rtl:mr-1 inline-flex px-1.5 py-0.5 rounded text-[10px] font-bold text-white" style="background-color: #C4A265;">{{ $t('a_current') }}</span>
                                    </td>
                                    ...
                                    <td class="px-4 py-2.5 text-right font-semibold" :class="p.id === payment.id ? 'text-emerald-700' : 'text-emerald-600'">{{ formatCurrency(p.amount) }}</td>
...
@media print {
    ...
    .bg-gray-50, .bg-amber-50\/60, thead tr { -webkit-print-color-adjust: exact; print-color-adjust: exact; }
}
</style>
