...
class InsuranceReportController extends Controller
{
    public function index(Request $request): Response
    {
        ...
        // Claims by Company (via patient_insurances → insurance_companies)
        $companyPerformance = InsuranceClaim::whereBetween('insurance_claims.created_at', [$dateFrom, $dateTo . ' 23:59:59'])
            ->join('patient_insurances', 'insurance_claims.patient_insurance_id', '=', 'patient_insurances.id')
            ->join('insurance_companies', 'patient_insurances.insurance_company_id', '=', 'insurance_companies.id')
            ->select(
                ...
                DB::raw('SUM(insurance_claims.total_amount) as total_claimed'),
                ...
                DB::raw('ROUND(SUM(insurance_claims.paid_amount) * 100.0 / NULLIF(SUM(insurance_claims.total_amount), 0), 1) as collection_rate')
            )
        ...
        $monthlyTrend = InsuranceClaim::where('created_at', '>=', now()->subMonths(6)->startOfMonth())
            ->select(
                ...
                DB::raw('SUM(total_amount) as claimed'),
        ...
        $activeInsured = PatientInsurance::where('is_active', true)
            ->where(function ($q) {
        ...
        $avgProcessingDays = InsuranceClaim::whereIn('status', ['paid', 'partially_paid'])
            ...
            ->selectRaw('ROUND(AVG(DATEDIFF(paid_at, submitted_at)), 0) as avg_days')
            ->value('avg_days');

        // Aging Analysis
        $aging = InsuranceClaim::whereIn('status', ['submitted', 'under_review', 'approved'])
            ->select(
                DB::raw("SUM(CASE WHEN DATEDIFF(NOW(), submitted_at) <= 30 THEN 1 ELSE 0 END) as within_30"),
                DB::raw("SUM(CASE WHEN DATEDIFF(NOW(), submitted_at) BETWEEN 31 AND 60 THEN 1 ELSE 0 END) as days_31_60"),
                DB::raw("SUM(CASE WHEN DATEDIFF(NOW(), submitted_at) BETWEEN 61 AND 90 THEN 1 ELSE 0 END) as days_61_90"),
                DB::raw("SUM(CASE WHEN DATEDIFF(NOW(), submitted_at) > 90 THEN 1 ELSE 0 END) as over_90")
            )
            ->first();
