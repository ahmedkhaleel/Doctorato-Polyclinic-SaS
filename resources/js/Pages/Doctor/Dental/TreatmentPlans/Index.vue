            ...
            <div class="relative z-10">
                <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-5">
                    ...
                    <div class="flex items-center gap-2 sm:gap-3 flex-wrap">
                        <div class="dental-card-enter bg-white/5 backdrop-blur-sm rounded-xl px-3 sm:px-4 py-2.5 border border-white/10 text-center min-w-[70px]" style="animation-delay:0.2s">
                            <p class="text-xl font-bold text-white">{{ quickStats.total }}</p>
                            <p class="text-[10px] text-gray-400">{{ isRtl ? 'الإجمالي' : 'Total' }}</p>
                        </div>
                        <div class="dental-card-enter bg-white/5 backdrop-blur-sm rounded-xl px-3 sm:px-4 py-2.5 border border-[#C4A265]/20 text-center min-w-[70px]" style="animation-delay:0.3s">
                            <p class="text-xl font-bold text-[#C4A265]">{{ quickStats.inProgress }}</p>
                            <p class="text-[10px] text-gray-400">{{ isRtl ? 'قيد التنفيذ' : 'Active' }}</p>
                        </div>
                        <div class="dental-card-enter bg-white/5 backdrop-blur-sm rounded-xl px-3 sm:px-4 py-2.5 border border-emerald-500/20 text-center min-w-[70px]" style="animation-delay:0.4s">
                            <p class="text-xl font-bold text-emerald-400">{{ quickStats.completed }}</p>
                            <p class="text-[10px] text-gray-400">{{ isRtl ? 'مكتمل' : 'Done' }}</p>
                        </div>
                    </div>
                </div>
                ...
                <!-- Date Range Filters -->
                <div class="flex flex-col sm:flex-row flex-wrap gap-3 max-w-2xl mt-3 dental-card-enter" style="animation-delay:0.2s">
            ...
            <div class="overflow-x-auto">
                <table class="min-w-full divide-y divide-gray-200">
                    ...
                    <tbody class="divide-y divide-gray-100">
                        <tr v-for="(plan, idx) in plans.data" :key="plan.id"
                            class="hover:bg-gray-50/60 transition-all duration-300"
                            :class="mounted ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-2'"
                            :style="{ transitionDelay: (0.25 + idx * 0.04) + 's' }">
