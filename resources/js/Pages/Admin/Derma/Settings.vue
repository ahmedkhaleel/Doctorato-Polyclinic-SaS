...
const form = useForm({
    ...
    name_ar: props.settings?.name_ar ?? 'الجلدية',
    name_en: props.settings?.name_en ?? 'Dermatology',
});

const saved = ref(false);
function submit() {
    form.post('/admin/derma/settings', { preserveScroll: true, onSuccess: () => { saved.value = true; setTimeout(() => saved.value = false, 3000); }});
}
...
<template>
    <div class="space-y-6 pb-10 max-w-3xl">
        <div class="bg-gradient-to-br from-amber-600 to-amber-500 rounded-2xl p-6 shadow-lg">
            <h1 class="text-2xl font-bold text-white">{{ t('Derma Settings', 'إعدادات الجلدية') }}</h1>
        </div>
        ...
        <form @submit.prevent="submit" class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 space-y-4">
            ...
            <div class="pt-2">
                <button :disabled="form.processing" class="px-5 py-2.5 bg-amber-600 hover:bg-amber-700 text-white rounded-xl font-semibold text-sm disabled:opacity-60">
                    {{ t('Save', 'حفظ') }}
                </button>
            </div>
        </form>
