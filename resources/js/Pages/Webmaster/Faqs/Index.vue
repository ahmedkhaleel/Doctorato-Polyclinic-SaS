...
watch(search, (val) => {
    ...
    searchTimeout = setTimeout(() => {
        router.get('/webmaster/faqs', { search: val || undefined }, {
            preserveState: true,
            replace: true,
        });
    }, 400);
                ...
                <div class="overflow-x-auto">
                    <table class="min-w-full divide-y divide-gray-200">
                        <thead class="bg-gray-50">
                            <tr>
                                <th class="px-6 py-3 ltr:text-left rtl:text-right text-xs font-medium text-gray-500 uppercase tracking-wider">{{ $t('a_question') }}</th>
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            <tr v-for="faq in faqs.data" :key="faq.id" class="hover:bg-gray-50">
                                <td class="px-6 py-4 text-sm font-medium text-gray-900 max-w-md truncate">{{ faq.question_en }}</td>
                                ...
                                <td class="px-6 py-4 whitespace-nowrap ltr:text-right rtl:text-left text-sm ltr:space-x-3 rtl:space-x-reverse rtl:space-x-3">
                                    <Link v-if="can('faqs.update')" :href="`/webmaster/faqs/${faq.id}/edit`" class="font-medium hover:underline" style="color: #C4A265;">{{ $t('a_edit') }}</Link>
