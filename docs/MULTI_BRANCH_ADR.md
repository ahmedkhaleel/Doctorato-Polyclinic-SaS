...
**Status:** Accepted (التصميم مُعتمَد — التنفيذ مؤجَّل بقرار صاحب المنتج)
**Date:** 2026-05-31
...
**قرارات مُثبّتة من صاحب المنتج (2026-05-31):**
1. ✅ **المريض مشترك** على مستوى المنشأة (ملف طبي واحد + `home_branch_id`)؛ المعاملات تُنسب لفرعها. (لا `branch_id` على `patients` ولا على سجلّاته المتبعة له.)
2. ⏸️ **التنفيذ مؤجَّل** — يُكتفى بهذا الـ ADR حالياً؛ يبدأ التنفيذ من المرحلة B0 عند صدور الإذن.

**قرارات النطاق المُثبّتة (2026-05-31) — تَنسخ أي افتراض سابق في §5:**
- ✅ **CRM مركزي مشترك**: `leads/campaigns` بلا Global Scope؛ `branch_id` اختياري للإسناد/التقارير فقط (يُنقل من الفئة 1 إلى المشترك).
- ✅ **الخدمات/الأسعار موحّدة**: لا جدول `Service↔Branch` ولا تسعير per-branch (يُلغي هذا الجزء من B5).
- ✅ **مرسِل إشعارات واحد للمنشأة**: قنوات/اعتمادات الإشعارات تبقى عامة (لا per-branch)؛ فقط `notification_logs.branch_id` للتحليلات.
- ✅ **المخزون لكل فرع + الموردون مشتركون**: `supplies/supply_transactions/purchase_orders` → `branch_id`؛ `suppliers/supply_categories` عامة.
- **الأثر:** تقليص النطاق وأقل مخاطرة (الفئة المشتركة تتوسّع، والفئة المربوطة تنكمش). التفاصيل التنفيذية الكاملة في `docs/MULTI_BRANCH_CHECKLIST.md`.
