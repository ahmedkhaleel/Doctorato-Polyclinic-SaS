        ...
        <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
            <div v-for="p in photos.data" :key="p.id" class="bg-white rounded-xl border border-gray-100 overflow-hidden">
                ...
                <div class="p-3">
                    ...
                    <div class="text-[10px] text-blue-900 uppercase mt-1">{{ p.category }} · {{ p.procedure?.name_ar || '' }}</div>
            ...
            <div class="bg-white rounded-2xl w-full max-w-md p-6">
                ...
                <form @submit.prevent="submit" class="space-y-3">
                    ...
                    <div class="flex justify-end gap-2 pt-2">
                        ...
                        <button :disabled="form.processing" class="px-5 py-2 rounded-lg bg-blue-900 text-white text-sm font-semibold">{{ t('Upload', 'رفع') }}</button>
                    </div>
                </form>
            </div>
        </div>
