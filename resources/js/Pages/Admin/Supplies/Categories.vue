                    ...
                    <div class="p-5">
                        ...
                        <div class="flex items-center justify-between pt-3 border-t border-gray-100">
                            ...
                            <div class="flex items-center gap-1">
                                <button v-if="can('supplies.update')" @click="openEdit(cat)" class="w-8 h-8 rounded-lg flex items-center justify-center text-gray-400 hover:text-[#1B365D] hover:bg-slate-50 transition-all duration-200" :aria-label="isRtl ? 'تعديل' : 'Edit'" :title="isRtl ? 'تعديل' : 'Edit'">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" /></svg>
                                </button>
                                <button v-if="can('supplies.delete')" @click="confirmDelete(cat.id)" class="w-8 h-8 rounded-lg flex items-center justify-center text-gray-400 hover:text-red-600 hover:bg-red-50 transition-all duration-200" :aria-label="isRtl ? 'حذف' : 'Delete'" :title="isRtl ? 'حذف' : 'Delete'">
                    ...
                    >
                        <div v-if="showModal" class="relative bg-white rounded-2xl shadow-xl max-w-lg w-full z-10 overflow-hidden">
                            ...
                            <div class="px-4 md:px-6 py-4 border-b border-gray-100 bg-gradient-to-r from-gray-50 to-white flex items-center justify-between">
                                ...
                                <button @click="showModal = false" class="w-8 h-8 rounded-lg flex items-center justify-center text-gray-400 hover:text-gray-600 hover:bg-gray-100 transition-all" :aria-label="isRtl ? 'إغلاق' : 'Close'" :title="isRtl ? 'إغلاق' : 'Close'">
