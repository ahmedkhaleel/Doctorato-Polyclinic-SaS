...
function submit() {
    form.post('/webmaster/post-categories', { forceFormData: true });
}
</script>

<template>
    <WebmasterLayout title="Create Post Category">
        <div class="space-y-6">
            <div class="flex items-center justify-between">
                <h1 class="text-2xl font-bold text-gray-800">{{ $t('a_create_post_category') }}</h1>
                <Link href="/webmaster/post-categories" class="text-sm text-gray-500 hover:text-gray-700">{{ $t('a_back_to_post_categories') }}</Link>
            </div>
            ...
            <form @submit.prevent="submit" class="max-w-4xl">
                <div class="bg-white rounded-lg shadow-sm p-6 space-y-5">
                    ...
                    <div>
                        ...
                        <textarea v-model="form.description_en" rows="4" class="doctorato-input w-full px-4 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-[#C4A265]/30 focus:border-transparent"></textarea>
                    ...
                    <div>
                        ...
                        <textarea v-model="form.description_ar" rows="4" dir="rtl" class="doctorato-input w-full px-4 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-[#C4A265]/30 focus:border-transparent"></textarea>
                    ...
                    <div class="flex space-x-3 pt-4 border-t border-gray-200">
                        ...
                        <Link href="/webmaster/post-categories" class="py-2.5 px-6 rounded-lg bg-gray-200 text-gray-700 text-sm font-medium hover:bg-gray-300 transition">{{ $t('a_cancel') }}</Link>
                    </div>
                </div>
            </form>
        </div>
