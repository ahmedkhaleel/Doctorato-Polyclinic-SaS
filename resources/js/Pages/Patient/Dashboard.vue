...
<template>
    <div>
        <!-- Welcome Card -->
        <div class="bg-gradient-to-r from-[#0f172a] to-[#1e293b] rounded-2xl p-6 lg:p-8 mb-6 text-white">
            <h1 class="text-2xl lg:text-3xl font-bold mb-2">
                {{ isRtl ? 'مرحباً بعودتك،' : 'Welcome back,' }} {{ patient?.full_name }}
            </h1>
            <p class="text-white/50 text-sm">
                {{ isRtl ? 'إليك نظرة عامة على حسابك' : 'Here is an overview of your account' }}
            </p>
        </div>
        ...
        <div v-if="dentalStats" class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 mb-6">
            ...
            <div class="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-4">
                <div class="bg-cyan-50/50 rounded-xl p-3 text-center">
                    <p class="text-xl font-bold text-cyan-700">{{ dentalStats.total_treatments }}</p>
                    <p class="text-[10px] text-gray-500 mt-0.5">{{ isRtl ? 'إجمالي العلاجات' : 'Total Treatments' }}</p>
                </div>
                <div class="bg-green-50/50 rounded-xl p-3 text-center">
                    <p class="text-xl font-bold text-green-600">{{ dentalStats.completed_treatments }}</p>
                    <p class="text-[10px] text-gray-500 mt-0.5">{{ isRtl ? 'مكتمل' : 'Completed' }}</p>
                </div>
                <div class="bg-purple-50/50 rounded-xl p-3 text-center">
                    <p class="text-xl font-bold text-purple-600">{{ dentalStats.active_plans }}</p>
            ...
            <div class="grid grid-cols-2 lg:grid-cols-4 gap-2">
                ...
                <Link :href="lp('/dental/treatment-plans')" class="flex items-center gap-2 p-2.5 rounded-xl bg-gray-50 hover:bg-purple-50 border border-transparent hover:border-purple-200 transition text-sm group">
                    <svg class="w-4 h-4 text-purple-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2m-3 7h3m-3 4h3m-6-4h.01M9 16h.01" /></svg>
                    <span class="text-xs font-medium text-gray-600 group-hover:text-purple-600">{{ isRtl ? 'الخطط' : 'Plans' }}</span>
                </Link>
                <Link :href="lp('/dental/xrays')" class="flex items-center gap-2 p-2.5 rounded-xl bg-gray-50 hover:bg-cyan-50 border border-transparent hover:border-cyan-200 transition text-sm group">
                    <svg class="w-4 h-4 text-cyan-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z" /></svg>
                    <span class="text-xs font-medium text-gray-600 group-hover:text-cyan-600">{{ isRtl ? 'الأشعة' : 'X-Rays' }}</span>
                </Link>
            </div>

            <!-- Next Dental Appointment -->
            <div v-if="nextDentalAppointment" class="mt-4 p-4 bg-gradient-to-r from-cyan-50 to-teal-50 rounded-xl border border-cyan-100">
                <div class="flex items-center justify-between">
                    <div class="flex items-center gap-3">
                        ...
                        <div>
                            <p class="text-sm font-semibold text-cyan-800">{{ isRtl ? 'موعدك القادم' : 'Your Next Appointment' }}</p>
            ...
            <div v-if="dentalLabOrders?.length" class="mt-3 space-y-2">
                ...
                <div v-for="lo in dentalLabOrders" :key="lo.id" class="flex items-center justify-between p-3 rounded-xl bg-green-50/50 border border-green-100">
                    <div class="flex items-center gap-2">
                        <svg class="w-4 h-4 text-green-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7" /></svg>
                        <span class="text-xs text-gray-700">{{ lo.item_type }} — {{ $localized(lo.doctor, 'name') }}</span>
                    </div>
                    <span class="text-xs text-green-600 font-medium px-2 py-0.5 bg-green-100 rounded-full">{{ isRtl ? 'جاهز' : 'Ready' }}</span>
                </div>
            </div>
        </div>
