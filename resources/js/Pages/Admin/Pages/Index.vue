...
<template>
    <AdminLayout :title="$t('a_pages')">
        <div class="space-y-6">
            <h1 class="text-xl md:text-2xl font-bold text-gray-800">{{ $t('a_static_pages') }}</h1>
                    ...
                    <table class="min-w-full divide-y divide-gray-200">
                        <thead class="bg-gray-50">
                            <tr>
                                ...
                                <th class="px-4 md:px-6 py-3 ltr:text-left rtl:ltr:text-right rtl:text-left text-xs font-medium text-gray-500 uppercase tracking-wider">{{ $t('a_slug') }}</th>
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            <tr v-for="page in pages" :key="page.id" class="hover:bg-gray-50">
                                <td class="px-4 md:px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">{{ $localized(page, 'title') }}</td>
                                <td class="px-4 md:px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ page.slug }}</td>
                            ...
                            <tr v-if="!pages || pages.length === 0">
                                <td colspan="3" class="px-4 md:px-6 py-8 text-center text-sm text-gray-500">{{ $t('a_no_pages_found') }}</td>
                            </tr>
                        </tbody>
                    </table>
                </div>
