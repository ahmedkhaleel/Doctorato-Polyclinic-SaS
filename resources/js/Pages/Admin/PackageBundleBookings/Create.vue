...
const stepLabels = computed(() => [
    t('a_select_patient_bundle'),
    t('a_doctor_assignment'),
    t('a_scheduling'),
    t('a_review_confirm'),
]);
...
function submit() {
    ...
    router.post('/admin/package-bundle-bookings', payload, {
        onError: (errs) => {
            errors.value = errs;
        ...
        <div class="space-y-6">
                    ...
                    <template v-for="step in totalSteps" :key="step">
                        <div class="flex items-center">
                            <div class="flex items-center justify-center w-8 h-8 rounded-full text-sm font-semibold transition-all duration-300"
                                 ...
                                 :style="currentStep >= step ? 'background-color: #C4A265;' : ''">
                        ...
                        <div v-if="step < totalSteps" class="w-8 sm:w-12 mx-2 sm:mx-3 h-px"
                             :class="currentStep > step ? 'bg-[#C4A265]' : 'bg-gray-200'"></div>
                    </template>
                </div>
            </div>
            ...
            <div v-show="currentStep === 1" class="space-y-6">
                ...
                <div class="bg-white rounded-lg shadow-sm p-6">
                    <h3 class="text-lg font-semibold text-gray-800 mb-4">{{ $t('a_select_patient') }}</h3>

                    <div v-if="selectedPatient" class="flex items-center justify-between p-4 border-2 rounded-lg" style="border-color: #C4A265; background-color: rgba(196,162,101,0.05);">
                        <div class="flex items-center gap-3">
                            <div class="w-10 h-10 rounded-full flex items-center justify-center text-white text-sm font-semibold" style="background-color: #C4A265;">
                                {{ selectedPatient.full_name?.charAt(0) }}
                            </div>
                            <div>
                                <div class="font-medium text-gray-900">{{ selectedPatient.full_name }}</div>
                                <div class="text-xs text-gray-500">{{ selectedPatient.phone }} <span v-if="selectedPatient.file_number" class="ml-2 font-mono" style="color: #C4A265;">#{{ selectedPatient.file_number }}</span></div>
                            </div>
                        </div>
                        <button @click="clearPatient" class="text-gray-400 hover:text-red-500 transition p-1" :aria-label="isRtl ? 'إغلاق' : 'Close'" :title="isRtl ? 'إغلاق' : 'Close'">
                    ...
                    <div v-else class="relative">
                        ...
                        <div v-if="showPatientDropdown && filteredPatients.length > 0"
                            ...
                            <button v-for="p in filteredPatients" :key="p.id"
                                    ...
                                    class="w-full flex items-center gap-3 px-4 py-3 ltr:text-left rtl:text-right hover:bg-gray-50 transition">
                                ...
                                <div>
                                    ...
                                    <div class="text-xs text-gray-400">{{ p.phone }} <span v-if="p.file_number" class="font-mono ml-1">#{{ p.file_number }}</span></div>
                                </div>
                            </button>
                        </div>
                ...
                <!-- Bundle Selection -->
                <div class="bg-white rounded-lg shadow-sm p-6">
                    <h3 class="text-lg font-semibold text-gray-800 mb-4">{{ $t('a_select_package_bundle') }}</h3>
                    ...
                    <div v-else class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                        <button v-for="bundle in bundles" :key="bundle.id"
                                ...
                                class="relative ltr:text-left rtl:text-right p-5 rounded-xl border-2 transition-all duration-200 hover:shadow-md"
                                :class="selectedBundleId === bundle.id ? 'border-[#C4A265] shadow-md' : 'border-gray-200 hover:border-gray-300'"
                                :style="selectedBundleId === bundle.id ? 'background-color: rgba(196,162,101,0.05);' : ''">
                            ...
                            <div class="flex items-center gap-2 mb-2">
                                <span class="text-lg font-bold" style="color: #C4A265;">{{ formatCurrency(bundle.total_price) }}</span>
                    ...
                    <div v-if="selectedBundle" class="mt-6 border-t border-gray-200 pt-4">
                        <h4 class="text-sm font-semibold text-gray-700 mb-3">{{ $t('a_included_services') }}</h4>
                        <div class="space-y-2">
                            <div v-for="bs in bundleServices" :key="bs.id"
                                ...
                                <div>
                                    ...
                                    <span class="text-gray-400 ml-2">({{ bs.sessions_count }} {{ bs.sessions_count === 1 ? 'session' : 'sessions' }})</span>
                                </div>
            ...
            <div v-show="currentStep === 2" class="space-y-6">
                <div class="bg-white rounded-lg shadow-sm p-6">
                    <h3 class="text-lg font-semibold text-gray-800 mb-2">{{ $t('a_assign_doctors') }}</h3>
                    <p class="text-sm text-gray-500 mb-6">{{ $t('a_assign_doctor') }}</p>
            ...
            <div v-show="currentStep === 3" class="space-y-6">
                <div v-for="(sa, sIndex) in serviceAssignments" :key="sa.package_bundle_service_id"
                    ...
                    <div class="flex items-center gap-3 mb-4 pb-3 border-b border-gray-200">
                        <div class="w-8 h-8 rounded-lg flex items-center justify-center text-white text-sm font-bold" style="background-color: #C4A265;">
                            {{ sIndex + 1 }}
                        </div>
                        <div>
                    ...
                    <div class="space-y-4">
                        <div v-for="(apt, aIndex) in (appointmentData[sIndex] || [])" :key="aIndex"
                             ...
                             :class="apt.start_time ? 'border-[#C4A265]/30 bg-[#C4A265]/5' : ''">
                            <div class="flex items-center justify-between mb-3">
                                <span class="text-sm font-semibold text-gray-700">Session {{ aIndex + 1 }}</span>
                                <span v-if="apt.start_time && apt.date" class="text-xs font-medium px-2.5 py-1 rounded-full text-white" style="background-color: #C4A265;">
                            ...
                            <div v-else-if="apt.date && apt.availableSlots.length > 0" class="mt-2">
                                <label class="block text-xs font-medium text-gray-500 mb-2">{{ $t('a_available_time_slots') }}</label>
                                <div class="flex flex-wrap gap-2">
                                    <button v-for="slot in apt.availableSlots" :key="slot.start"
                                            ...
                                            :style="apt.start_time === slot.start ? 'background-color: #C4A265;' : ''">
            ...
            <div v-show="currentStep === 4" class="space-y-6">
                <div class="bg-white rounded-lg shadow-sm p-6">
                    <h3 class="text-lg font-semibold text-gray-800 mb-6">{{ $t('a_review_confirm') }}</h3>

                    <!-- Patient Summary -->
                    <div class="mb-6 p-4 bg-gray-50 rounded-xl">
                        <h4 class="text-xs font-semibold text-gray-400 uppercase tracking-wider mb-2">{{ $t('a_patient') }}</h4>
                        <div class="flex items-center gap-3">
                            <div class="w-10 h-10 rounded-full flex items-center justify-center text-white text-sm font-semibold" style="background-color: #C4A265;">
                                {{ selectedPatient?.full_name?.charAt(0) }}
                            </div>
                    ...
                    <!-- Bundle Summary -->
                    <div class="mb-6 p-4 bg-gray-50 rounded-xl">
                        <h4 class="text-xs font-semibold text-gray-400 uppercase tracking-wider mb-2">{{ $t('a_bundle') }}</h4>
                        <div class="flex items-center justify-between">
                            <div>
                    ...
                    <div class="mb-6">
                        <h4 class="text-xs font-semibold text-gray-400 uppercase tracking-wider mb-3">{{ $t('a_appointments_schedule') }}</h4>
                        <div class="space-y-4">
                            <div v-for="(sa, sIndex) in serviceAssignments" :key="sa.package_bundle_service_id"
                                ...
                                <div class="px-4 py-3 bg-gray-50 flex items-center justify-between">
                                    <div>
                                        ...
                                        <span class="text-gray-400 text-xs ml-2">({{ sa.sessions_count }} sessions)</span>
                                    </div>
                                    <span class="font-semibold text-sm" style="color: #C4A265;">{{ formatCurrency(sa.bundle_price) }}</span>
                                </div>
                    ...
                    <div v-if="Object.keys(errors).length > 0" class="bg-red-50 border border-red-200 rounded-lg p-4 text-sm text-red-700">
                        <p class="font-semibold mb-1">{{ $t('a_fix_errors') }}</p>
            ...
            <div class="bg-white rounded-lg shadow-sm p-4 flex items-center justify-between">
                ...
                <button v-else
                    ...
                    {{ processing ? $t('a_creating') : $t('a_confirm_booking') }}
                </button>
            </div>
        </div>
