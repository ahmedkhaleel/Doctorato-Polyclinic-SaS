                ...
                <div class="overflow-x-auto">
                    <table class="min-w-full divide-y divide-gray-200">
                        <thead class="bg-gray-50">
                            <tr>
                                <th class="px-4 md:px-6 py-3 ltr:text-left rtl:text-right text-xs font-medium text-gray-500 uppercase tracking-wider">Booking</th>
                                ...
                                <th class="px-4 md:px-6 py-3 ltr:text-left rtl:text-right text-xs font-medium text-gray-500 uppercase tracking-wider">{{ $t('a_bundle') }}</th>
                                <th class="px-4 md:px-6 py-3 ltr:text-left rtl:text-right text-xs font-medium text-gray-500 uppercase tracking-wider">{{ $t('a_price') }}</th>
                                <th class="px-4 md:px-6 py-3 text-center text-xs font-medium text-gray-500 uppercase tracking-wider">{{ $t('a_progress') }}</th>
                                <th class="px-4 md:px-6 py-3 text-center text-xs font-medium text-gray-500 uppercase tracking-wider">{{ $t('a_status') }}</th>
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            <tr v-for="b in bookings.data" :key="b.id" class="hover:bg-gray-50">
                                ...
                                <td class="px-4 md:px-6 py-4 whitespace-nowrap text-sm text-gray-900">{{ b.package_bundle?.name_en || '-' }}</td>
                                <td class="px-4 md:px-6 py-4 whitespace-nowrap">
                                ...
                                <td class="px-4 md:px-6 py-4 whitespace-nowrap text-right text-sm">
                                    <Link :href="`/admin/package-bundle-bookings/${b.id}`"
                                          class="p-2 rounded-lg text-gray-400 hover:text-[#1B365D] hover:bg-slate-50 transition-all duration-200 inline-block"
                                          title="View">
                            ...
                            <tr v-if="!bookings.data || bookings.data.length === 0">
                                <td colspan="7" class="px-4 md:px-6 py-8 text-center text-sm text-gray-500">No bundle bookings found.</td>
                            </tr>
                        </tbody>
                    </table>
                </div>
