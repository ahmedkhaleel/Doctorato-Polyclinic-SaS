...
function submit() {
    form.post('/webmaster/tags', { forceFormData: true });
}
</script>

<template>
    <WebmasterLayout title="Create Tag">
        <div class="space-y-6">
            <div class="flex items-center justify-between">
                <h1 class="text-2xl font-bold text-gray-800">{{ $t('a_create_tag') }}</h1>
                <Link href="/webmaster/tags" class="text-sm text-gray-500 hover:text-gray-700">{{ $t('a_back_to_tags') }}</Link>
            </div>
            ...
            <form @submit.prevent="submit" class="max-w-4xl">
                <div class="bg-white rounded-lg shadow-sm p-6 space-y-5">
                    ...
                    <div class="flex space-x-3 pt-4 border-t border-gray-200">
                        ...
                        >
                            {{ form.processing ? $t('a_saving') : $t('a_create_tag') }}
                        </button>
                        <Link href="/webmaster/tags" class="py-2.5 px-6 rounded-lg bg-gray-200 text-gray-700 text-sm font-medium hover:bg-gray-300 transition">{{ $t('a_cancel') }}</Link>
                    </div>
                </div>
            </form>
        </div>
