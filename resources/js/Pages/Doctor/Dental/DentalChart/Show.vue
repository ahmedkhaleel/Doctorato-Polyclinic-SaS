...
<template>
                    ...
                    <div class="flex items-center gap-2 flex-wrap justify-start sm:justify-end">
                        ...
                        <button @click="showDeciduousTeeth = !showDeciduousTeeth; selectedTooth = null;"
                            ...
                            :class="showDeciduousTeeth
                                ? 'text-amber-300 bg-[#C4A265]/15 border-[#C4A265]/20 hover:bg-[#C4A265]/25'
                        ...
                        <Link :href="`/doctor/dental/treatment-plans?patient_id=${patient.id}`"
                            class="inline-flex items-center px-3 py-2 text-xs font-medium text-slate-300 bg-[#1B365D]/15 rounded-xl hover:bg-[#1B365D]/25 border border-[#1B365D]/20 transition-all">
                        ...
                        <div class="flex items-center gap-3 mb-3">
                            ...
                            <div class="ms-auto flex items-center gap-1.5 flex-wrap">
                                <button @click="showQuickTreatment = !showQuickTreatment"
                                    class="inline-flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium text-[#1B365D] bg-slate-50 hover:bg-slate-100 rounded-lg border border-slate-200 transition-all"
                                    :class="showQuickTreatment ? 'ring-2 ring-slate-300' : ''">
                        ...
                        <div class="p-4 space-y-3">
                            ...
                            <div class="flex justify-end gap-2 pt-1">
                                ...
                                <button @click="saveQuickTreatment" :disabled="savingTreatment || !quickTreatmentForm.treatment_type"
                                    class="px-5 py-2 bg-[#1B365D] text-white rounded-lg text-xs font-semibold hover:bg-[#1B365D] disabled:opacity-40 transition-all shadow-sm flex items-center gap-1.5">
                        ...
                        <div v-for="tr in getToothTreatments(selectedTooth)" :key="tr.id" class="px-3 sm:px-5 py-3 flex items-center justify-between hover:bg-gray-50/50 transition-colors">
                            <div class="flex items-center gap-2.5">
                                <div class="w-1.5 h-1.5 rounded-full flex-shrink-0" :class="{
                                    'bg-emerald-500': tr.status === 'completed',
                            ...
                            <span class="text-[10px] font-semibold px-2 py-0.5 rounded-full" :class="{
                                'bg-emerald-50 text-emerald-700': tr.status === 'completed',
    ...
    <Teleport to="body">
        <Transition enter-active-class="transition-all duration-300 ease-out" enter-from-class="opacity-0" enter-to-class="opacity-100"
            ...
            <div v-if="showModal" v-focus-trap="() => (showModal = false)" role="dialog" aria-modal="true" class="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4" @click.self="showModal = false">
                <div class="bg-white rounded-2xl shadow-2xl w-full max-w-lg overflow-hidden">
                    <div class="bg-gradient-to-r from-[#1B365D] to-[#0F2444] px-4 sm:px-6 py-4 flex items-center justify-between">
