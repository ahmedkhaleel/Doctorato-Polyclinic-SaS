...
<template>
    <div ref="wrapperRef" class="relative">
        ...
        <div v-if="selectedPatient" class="flex items-center gap-2 px-3.5 py-2.5 border border-[#C4A265]/40 bg-[#C4A265]/5 rounded-xl">
            <div class="flex-1 min-w-0">
                <span class="text-sm font-semibold text-gray-800">{{ selectedPatient.full_name }}</span>
                <span v-if="selectedPatient.file_number" class="text-xs font-mono text-[#C4A265] ms-2">#{{ selectedPatient.file_number }}</span>
        ...
        <div v-else class="relative">
            <input
                ...
                class="w-full px-3.5 py-2.5 ps-10 border border-gray-200 rounded-xl text-sm focus:ring-2 focus:ring-[#C4A265]/30 focus:border-[#C4A265] bg-white"
            ...
            <svg v-if="isLoading" class="w-4 h-4 text-[#C4A265] absolute end-3.5 top-1/2 -translate-y-1/2 animate-spin" fill="none" viewBox="0 0 24 24"><circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4" /><path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z" /></svg>
        </div>
        ...
        <Transition enter-active-class="transition ease-out duration-150" enter-from-class="opacity-0 translate-y-1" enter-to-class="opacity-100 translate-y-0" leave-active-class="transition ease-in duration-100" leave-from-class="opacity-100" leave-to-class="opacity-0 translate-y-1">
            <ul v-if="isOpen && results.length > 0" class="absolute z-50 w-full mt-1 bg-white border border-gray-200 rounded-xl shadow-lg max-h-60 overflow-y-auto">
                <li v-for="p in results" :key="p.id" @click="selectPatient(p)" class="flex items-center gap-3 px-4 py-2.5 hover:bg-[#C4A265]/5 cursor-pointer transition group">
                    <div class="w-8 h-8 rounded-full bg-gradient-to-br from-[#C4A265]/20 to-[#C4A265]/5 flex items-center justify-center flex-shrink-0">
                        <span class="text-xs font-bold text-[#C4A265]">{{ p.full_name?.charAt(0) }}</span>
                    </div>
                    <div class="flex-1 min-w-0">
                        <div class="text-sm font-semibold text-gray-800 group-hover:text-[#C4A265] truncate">{{ p.full_name }}</div>
