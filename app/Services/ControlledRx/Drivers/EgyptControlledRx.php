...
/**
 * Egypt national controlled-substance platform driver (STUB). The regulator
 ...
 */
class EgyptControlledRx implements ControlledRxGatewayInterface
{
    ...
    {
        return ! empty(Setting::get('controlled_rx_egypt_api_key', ''));
    }

    public function submit(ControlledPrescription $rx): ControlledRxResult
    {
        if (! $this->isConfigured()) {
            return ControlledRxResult::failed('Egypt controlled-Rx platform not configured (facility credentials required).');
        }

        // TODO: implement the live Egypt regulator API call once credentials are available.
        return ControlledRxResult::failed('Egypt controlled-Rx integration not yet implemented.');
    }
