                            ...
                            <tr v-for="(rule, idx) in rules" :key="rule.id"
                                ...
                                <td class="px-4 md:px-6 py-4 ltr:text-right rtl:text-left">
                                    ...
                                    <div class="flex items-center justify-end gap-1.5 sm:hidden">
                                        <button v-if="can('leads.update')" @click="openEdit(rule)"
                                            class="p-2 rounded-lg text-gray-400 hover:text-[#C4A265] transition-all duration-200" :aria-label="isRtl ? 'تعديل' : 'Edit'" :title="isRtl ? 'تعديل' : 'Edit'">
                                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" /></svg>
                                        </button>
                                        <button v-if="can('leads.update')" @click="deleteRule(rule.id)"
                                            class="p-2 rounded-lg text-gray-400 hover:text-red-500 transition-all duration-200" :aria-label="isRtl ? 'حذف' : 'Delete'" :title="isRtl ? 'حذف' : 'Delete'">
                                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" /></svg>
                                        </button>
                                    </div>
                                </td>
