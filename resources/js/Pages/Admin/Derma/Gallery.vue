...
<template>
    <div class="space-y-6 pb-10">
            ...
            <div class="relative p-4 md:p-7 flex flex-col md:flex-row md:items-center gap-4 md:gap-5 justify-between">
                <div class="flex items-start gap-3 md:gap-4 min-w-0">
                    ...
                    <div class="min-w-0">
                        ...
                        <p class="text-xs md:text-sm text-white/70 mt-1">{{ t('Dermatology photo documentation', 'توثيق الصور الجلدية') }}</p>
                    </div>
                </div>
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-slate-200 p-4 flex flex-wrap gap-3">
            <select v-model="patientId" class="doctorato-input px-4 py-2.5 border border-slate-200 rounded-xl text-sm">
        ...
        <div v-if="photos.data.length" class="grid grid-cols-2 md:grid-cols-4 gap-3 md:gap-4">
            <div v-for="p in photos.data" :key="p.id" class="bg-white rounded-xl border border-[#C4A265]/20 overflow-hidden group hover:shadow-lg hover:shadow-[#C4A265]/10 transition">
                <img :src="p.url" class="w-full h-40 object-cover" alt="" />
                <div class="p-3">
