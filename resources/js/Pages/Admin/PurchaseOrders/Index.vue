...
<template>
    <div class="p-4 md:p-6 max-w-7xl mx-auto">
        ...
        <div class="flex items-center justify-between mb-6">
            <div>
                <h1 class="text-xl md:text-2xl font-bold text-gray-800">{{ isRtl ? 'أوامر الشراء' : 'Purchase Orders' }}</h1>
                <p class="text-gray-500 text-sm mt-1">{{ isRtl ? 'إدارة أوامر الشراء والتوريدات' : 'Manage purchase orders and deliveries' }}</p>
            </div>
            <Link href="/admin/purchase-orders/create" class="inline-flex items-center gap-2 px-5 py-2.5 bg-[#1B365D] text-white rounded-xl hover:bg-[#1B365D] transition font-medium text-sm">
        ...
        <div class="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
            ...
            <div class="bg-white rounded-2xl border border-gray-100 p-4">
                <div class="flex items-center gap-3">
                    <div class="w-10 h-10 bg-slate-50 rounded-xl flex items-center justify-center">
                        <svg class="w-5 h-5 text-[#1B365D]" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8c1.11 0 2.08.402 2.599 1M12 8V7m0 1v8m0 0v1m0-1c-1.11 0-2.08-.402-2.599-1"/></svg>
                    </div>
                    <div>
                ...
                <tbody class="divide-y divide-gray-50">
                    <tr v-for="o in orders.data" :key="o.id" class="hover:bg-gray-50/50">
                        ...
                        <td class="px-4 py-3 text-center">
                            <span class="px-2 py-0.5 bg-slate-50 text-[#1B365D] rounded-full text-xs font-medium">{{ o.items_count }}</span>
                        </td>
                        ...
                        <td class="px-4 py-3 text-center">
                            <Link :href="`/admin/purchase-orders/${o.id}`" class="p-1.5 text-gray-400 hover:text-[#1B365D] hover:bg-slate-50 rounded-lg transition inline-flex">
        ...
        <div v-if="orders.links && orders.last_page > 1" class="flex justify-center gap-1 mt-6">
            <Link v-for="link in orders.links" :key="link.label" :href="link.url || '#'" class="px-3 py-1.5 rounded-lg text-sm" :class="link.active ? 'bg-[#1B365D] text-white' : 'text-gray-500 hover:bg-gray-100'" v-html="link.label" preserve-state />
        </div>
    </div>
</template>
