...
function submit() {
    form.post('/admin/tags', { forceFormData: true });
}
</script>

<template>
    <AdminLayout :title="$t('a_create_tag')">
        <div class="space-y-6">
            <div class="flex items-center justify-between">
                <h1 class="text-xl md:text-2xl font-bold text-gray-800">{{ $t('a_create_tag') }}</h1>
            ...
            <form @submit.prevent="submit" class="max-w-4xl">
                <div class="bg-white rounded-lg shadow-sm p-4 md:p-6 space-y-5">
                    ...
                    <div class="flex space-x-3 pt-4 border-t border-gray-200">
                        ...
                        <Link href="/admin/tags" class="py-2.5 px-4 md:px-6 rounded-lg bg-gray-200 text-gray-700 text-sm font-medium hover:bg-gray-300 transition">
                            {{ $t('a_cancel') }}
                        </Link>
                    </div>
                </div>
