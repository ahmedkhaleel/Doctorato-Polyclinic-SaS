...
function deleteSlide(id) {
    confirm(t('a_confirm_delete_slide'), () => {
...
function onDrop(index) {
    ...
    router.post('/admin/slider/update-order', { slides: ordered }, {
        preserveState: true,
    });
...
<template>
    <AdminLayout :title="$t('a_hero_slider')">
        <div class="space-y-6">
            ...
            <div v-else class="bg-white rounded-xl shadow-sm p-12 text-center">
                ...
                <p class="text-gray-500 text-sm mb-6">{{ $t('a_add_slides_hint') }}</p>
                <Link
                    v-if="can('settings.update')"
                    href="/admin/slider/create"
