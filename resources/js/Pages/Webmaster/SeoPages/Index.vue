...
<template>
    <WebmasterLayout title="SEO Pages">
        <div class="space-y-6">
            <!-- Header -->
            <div class="flex items-center justify-between">
                <div>
                    <h1 class="text-2xl font-bold text-gray-800">{{ $t('a_seo_pages') }}</h1>
                    <p class="text-sm text-gray-500 mt-1">{{ $t('a_manage_seo_desc') }}</p>
                </div>
                <div class="flex items-center gap-2 px-3 py-1.5 rounded-lg bg-amber-50 border border-amber-200">
                    ...
                    <span class="text-xs font-medium text-amber-700">{{ seoPages?.length || 0 }} pages configured</span>
                </div>
            </div>
                ...
                <table class="w-full text-sm">
                    <thead>
                        <tr class="bg-gray-50 border-b border-gray-200">
                            ...
                            <th class="px-6 py-3.5 ltr:text-center rtl:text-center text-xs font-semibold text-gray-500 uppercase tracking-wider">{{ $t('a_indexable') }}</th>
                            <th class="px-6 py-3.5 ltr:text-center rtl:text-center text-xs font-semibold text-gray-500 uppercase tracking-wider">{{ $t('a_action') }}</th>
                        </tr>
                    </thead>
                    <tbody class="divide-y divide-gray-100">
                        <tr v-for="page in seoPages" :key="page.id" class="hover:bg-gray-50/50 transition-colors">
                            ...
                            <td class="px-6 py-4 text-center">
                                <Link
                                    :href="`/webmaster/seo-pages/${page.id}/edit`"
