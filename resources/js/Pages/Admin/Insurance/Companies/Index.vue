...
<template>
    <div class="p-6 max-w-7xl mx-auto">
        ...
        <div class="flex items-center justify-between mb-6">
            ...
            <button @click="openCreate" class="inline-flex items-center gap-2 px-5 py-2.5 bg-cyan-600 text-white rounded-xl hover:bg-cyan-700 transition font-medium">
        ...
        <div class="flex flex-wrap gap-3 mb-6">
            ...
            <select v-model="statusFilter" @change="applyFilters" class="px-4 py-2.5 border border-gray-200 rounded-xl text-sm focus:ring-cyan-500 focus:border-cyan-500">
            ...
            <div v-for="company in companies.data" :key="company.id" class="bg-white rounded-2xl border border-gray-100 p-5 hover:shadow-lg transition-all group">
                <div class="flex items-start justify-between mb-4">
                    <div class="flex items-center gap-3">
                        ...
                        <div v-else class="w-12 h-12 rounded-xl bg-cyan-50 flex items-center justify-center">
                ...
                <div class="flex items-center justify-between mt-4 pt-4 border-t border-gray-50">
                    ...
                    <div class="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition">
                        <button @click="openEdit(company)" class="p-1.5 text-gray-400 hover:text-cyan-600 hover:bg-cyan-50 rounded-lg transition">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/></svg>
                        </button>
                        <button @click="deleteCompany(company)" class="p-1.5 text-gray-400 hover:text-red-600 hover:bg-red-50 rounded-lg transition">
                    ...
                    <form @submit.prevent="submit" class="space-y-4">
                        <div class="grid grid-cols-2 gap-4">
                            <div>
                                ...
                                <input v-model="form.name_ar" type="text" required class="w-full px-3 py-2.5 border border-gray-200 rounded-xl text-sm focus:ring-cyan-500 focus:border-cyan-500" />
                            </div>
                            <div>
                                <label class="block text-sm font-medium text-gray-700 mb-1">{{ isRtl ? 'الاسم بالإنجليزي' : 'Name (English)' }} *</label>
                                <input v-model="form.name_en" type="text" required class="w-full px-3 py-2.5 border border-gray-200 rounded-xl text-sm focus:ring-cyan-500 focus:border-cyan-500" />
                            </div>
                        </div>
                        ...
                        <div class="grid grid-cols-2 gap-4">
                            <div>
                                ...
                                <input v-model="form.code" type="text" :placeholder="isRtl ? 'مثال: BUPA' : 'e.g., BUPA'" class="w-full px-3 py-2.5 border border-gray-200 rounded-xl text-sm focus:ring-cyan-500 focus:border-cyan-500 font-mono" />
                            </div>
                            <div>
                                <label class="block text-sm font-medium text-gray-700 mb-1">{{ isRtl ? 'الهاتف' : 'Phone' }}</label>
                                <input v-model="form.phone" type="tel" class="w-full px-3 py-2.5 border border-gray-200 rounded-xl text-sm focus:ring-cyan-500 focus:border-cyan-500" />
                            </div>
                        </div>
                        ...
                        <div class="grid grid-cols-2 gap-4">
                            <div>
                                ...
                                <input v-model="form.email" type="email" class="w-full px-3 py-2.5 border border-gray-200 rounded-xl text-sm focus:ring-cyan-500 focus:border-cyan-500" />
                            </div>
                            <div>
                                <label class="block text-sm font-medium text-gray-700 mb-1">{{ isRtl ? 'جهة التواصل' : 'Contact Person' }}</label>
                                <input v-model="form.contact_person" type="text" class="w-full px-3 py-2.5 border border-gray-200 rounded-xl text-sm focus:ring-cyan-500 focus:border-cyan-500" />
                            </div>
                        </div>

                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-1">{{ isRtl ? 'العنوان' : 'Address' }}</label>
                            <textarea v-model="form.address" rows="2" class="w-full px-3 py-2.5 border border-gray-200 rounded-xl text-sm focus:ring-cyan-500 focus:border-cyan-500" />
                        </div>

                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-1">{{ isRtl ? 'ملاحظات' : 'Notes' }}</label>
                            <textarea v-model="form.notes" rows="2" class="w-full px-3 py-2.5 border border-gray-200 rounded-xl text-sm focus:ring-cyan-500 focus:border-cyan-500" />
                        </div>
