...
function submit() { form.post(`/admin/templates/${props.template.id}`); }
        ...
        <div class="space-y-6">
            ...
            <div :class="mounted ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'" class="transition-all duration-500 flex items-center gap-4">
                ...
                <div>
                    ...
                    <p class="text-xs text-gray-400 mt-0.5">Modify "{{ template.name }}"</p>
                </div>
            </div>
            ...
            <form @submit.prevent="submit" class="grid grid-cols-1 lg:grid-cols-5 gap-6">
                ...
                <div class="lg:col-span-3 space-y-6">
                    <!-- Template Details -->
                    <div :class="mounted ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'" class="transition-all duration-500 delay-100 bg-white rounded-2xl shadow-sm border border-gray-100 p-8 relative overflow-hidden">
                    ...
                    <div :class="mounted ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'" class="transition-all duration-500 delay-200 bg-white rounded-2xl shadow-sm border border-gray-100 p-8">
                        <div class="space-y-3">
                            <button type="submit" :disabled="form.processing"
                                ...
                                <svg v-if="form.processing" class="w-4 h-4 animate-spin" fill="none" viewBox="0 0 24 24"><circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"/><path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z"/></svg>{{ $t('a_update_template') }}</button>
