...
function checkDuplicates() {
    ...
    debounceTimer = setTimeout(async () => {
        ...
        try {
            ...
            const res = await fetch(`/admin/leads-check-duplicate?${params.toString()}`);
...
watch(() => form.phone, checkDuplicates);
watch(() => form.email, checkDuplicates);

function submit() {
    form.post('/admin/leads');
}
...
const statusLabels = computed(() => ({
    ...
    appointment_booked: t('a_lead_appt_booked'), converted: t('a_lead_converted'), lost: t('a_lead_lost'),
}));
...
const filteredSources = computed(() => {
    ...
    return props.sources.filter(s => (s[`name_${locale.value}`] || s.name_en || '').toLowerCase().includes(q));
});
const selectedSourceLabel = computed(() => {
    if (!form.lead_source_id) return '';
    const found = props.sources?.find(s => s.id === form.lead_source_id);
    return found ? (found[`name_${locale.value}`] || found.name_en) : '';
});
...
const sectionsVisible = ref([false, false, false, false, false, false]);
...
<template>
    <AdminLayout :title="$t('a_new_lead')">
        <div class="space-y-6 pb-28">
            ...
            >
                <Link href="/admin/leads" class="flex items-center justify-center w-10 h-10 rounded-xl bg-white border border-gray-200 text-gray-400 hover:text-[#C4A265] hover:border-[#C4A265]/40 transition-all duration-200 shadow-sm">
            ...
            <form @submit.prevent="submit" class="space-y-6">
                ...
                >
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
                        ...
                        <!-- Phone 2 -->
                        <div>
                            <label class="block text-xs font-semibold text-gray-500 uppercase tracking-wide mb-2">{{ $t('a_phone_2') }}</label>
                            <div class="relative">
                                <div class="absolute inset-y-0 ltr:left-0 rtl:right-0 flex items-center ltr:pl-3.5 rtl:pr-3.5 pointer-events-none">
                                    <svg class="w-4 h-4 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.493 1.498a1 1 0 01.684.949V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z" /></svg>
                                </div>
                ...
                >
                    <div v-if="duplicates.length" class="bg-gradient-to-r from-amber-50 to-amber-50 border border-amber-200 rounded-2xl p-4 md:p-6 shadow-sm">
                        <div class="flex items-start gap-4">
                            ...
                            <div class="flex-1">
                                <h4 class="text-sm font-bold text-amber-800">{{ $t('a_possible_duplicates') }}</h4>
                                <p class="text-xs text-amber-600 mt-1">{{ $t('a_duplicate_match_text') }}</p>
                                <div class="mt-3 space-y-2">
                                    <Link v-for="dup in duplicates" :key="dup.id" :href="`/admin/leads/${dup.id}`" target="_blank"
                                        class="flex items-center justify-between p-3 bg-white rounded-xl border border-amber-100 hover:border-amber-300 hover:shadow-sm transition-all duration-200"
                                    >
                                        <div>
                                            <span class="text-sm font-semibold text-gray-800">{{ dup.full_name }}</span>
                                            <span class="text-xs text-gray-500 ltr:ml-2 rtl:mr-2">{{ dup.phone || dup.email }}</span>
                                        </div>
                                        <span class="text-[10px] px-2.5 py-1 rounded-full bg-amber-100 text-amber-700 font-medium capitalize">{{ statusLabels[dup.status] || dup.status }}</span>
                                    </Link>
                                </div>
                                <p class="text-xs text-amber-600 mt-3">{{ $t('a_duplicate_still_create') }}</p>
                            </div>
                        </div>
                    </div>
                </Transition>
                            ...
                            <div v-else>
                                ...
                                >
                                    <div v-if="sourceSearchOpen" class="absolute z-30 mt-1 w-full bg-white border border-gray-200 rounded-xl shadow-lg max-h-48 overflow-y-auto">
                                        ...
                                        >{{ s[`name_${locale}`] || s.name_en }}</button>
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-3 gap-3">
                                <label
                                    v-for="(config, val) in {
                                        ...
                                        '2': { label: $t('a_priority_warm'), color: 'amber', icon: 'M12 3v1m0 16v1m9-9h-1M4 12H3m15.364 6.364l-.707-.707M6.343 6.343l-.707-.707m12.728 0l-.707.707M6.343 17.657l-.707.707M16 12a4 4 0 11-8 0 4 4 0 018 0z' },
                                        '3': { label: $t('a_priority_cold'), color: 'blue', icon: 'M20.354 15.354A9 9 0 018.646 3.646 9.003 9.003 0 0012 21a9.003 9.003 0 008.354-5.646z' }
                                    }"
                                    :key="val"
                                    :class="[
                                        form.priority === val
                ...
                >
                    <h3 class="text-sm font-semibold text-gray-500 uppercase tracking-wider mb-6 flex items-center gap-2">
                        ...
                        <span v-if="form.interested_services.length" class="ml-auto text-xs font-medium text-[#C4A265] bg-[#C4A265]/10 px-2.5 py-1 rounded-full">{{ form.interested_services.length }} {{ $t('a_selected') }}</span>
                    </h3>
                    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
                        ...
                        >
                            ...
                            <span class="text-sm font-medium" :class="form.interested_services.includes(service.id) ? 'text-[#C4A265]' : 'text-gray-600'">{{ service[`name_${locale}`] || service.name_en }}</span>
                        </button>
                    </div>
                </div>
                ...
                <div class="flex items-center justify-between px-4 md:px-6 lg:px-8 py-4 max-w-screen-2xl mx-auto">
                    <div class="flex items-center gap-2 text-sm text-gray-500">
                        <div v-if="checkingDuplicates" class="flex items-center gap-2">
                            ...
                            <span>{{ $t('a_checking_duplicates') }}</span>
                        </div>
                        <div v-else-if="duplicates.length" class="flex items-center gap-2 text-amber-600">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-2.5L13.732 4c-.77-.833-1.964-.833-2.732 0L4.082 16.5c-.77.833.192 2.5 1.732 2.5z" /></svg>
                            <span>{{ duplicates.length }} {{ $t('a_possible_duplicates_detected') }}</span>
                        </div>
                    </div>
                    <div class="flex items-center gap-3">
                        <Link href="/admin/leads" class="px-5 py-2.5 text-sm font-medium text-gray-600 hover:text-gray-800 bg-gray-100 hover:bg-gray-200 rounded-xl transition-all duration-200">{{ $t('a_cancel') }}</Link>
                        <button
                            @click="submit"
                        ...
                        >
                            <span v-if="form.processing" class="flex items-center gap-2">
                                <svg class="w-4 h-4 animate-spin" fill="none" viewBox="0 0 24 24"><circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4" /><path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z" /></svg>{{ $t('a_creating') }}</span>
                            <span v-else>{{ $t('a_create_lead') }}</span>
                        </button>
                    </div>
                </div>
            </div>
