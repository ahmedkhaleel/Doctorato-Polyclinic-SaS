...
<template>
    <div class="max-w-4xl mx-auto py-6 px-4 sm:px-6">
        <!-- Hero Header -->
        <div class="dental-hero-enter relative mb-8 rounded-2xl overflow-hidden bg-gradient-to-br from-cyan-600 via-cyan-700 to-teal-800 p-8 text-white shadow-xl">
            ...
            <div class="relative z-10 flex items-center gap-3">
                ...
                <div>
                    <h1 class="text-2xl font-bold">{{ isRtl ? 'مخطط الأسنان' : 'Dental Chart' }}</h1>
                    <p class="text-cyan-100/80 text-sm mt-0.5">{{ isRtl ? 'ابحث عن مريض لفتح مخطط أسنانه' : 'Search for a patient to open their dental chart' }}</p>
                </div>
            </div>
        </div>
        ...
        <div class="dental-card-enter relative mb-8" style="animation-delay: 0.15s">
            <div class="relative">
                ...
                <input v-model="searchQuery" type="text"
                    ...
                    class="w-full border border-gray-200 rounded-2xl py-4 text-base bg-white shadow-sm focus:border-cyan-400 focus:ring-2 focus:ring-cyan-200/60 focus:shadow-lg transition-all duration-300"
                    :class="isRtl ? 'pr-13 pl-4' : 'pl-13 pr-4'"
                />
                <div v-if="searching" class="absolute top-1/2 -translate-y-1/2" :class="isRtl ? 'left-5' : 'right-5'">
                    <svg class="w-5 h-5 animate-spin text-cyan-500" fill="none" viewBox="0 0 24 24"><circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4" /><path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z" /></svg>
                </div>
            </div>
        </div>
        ...
        <div v-if="patients.length > 0" class="mb-8">
            ...
            <div class="space-y-2">
                <button v-for="patient in patients" :key="patient.id"
                    ...
                    class="w-full flex items-center gap-4 p-4 bg-white rounded-xl border border-gray-200 hover:border-cyan-300 hover:shadow-md hover:bg-cyan-50/30 transition-all group text-start"
                >
                    <div class="w-12 h-12 rounded-xl bg-gradient-to-br from-cyan-100 to-cyan-50 flex items-center justify-center flex-shrink-0 group-hover:from-cyan-200 transition-all">
                        ...
                        <span v-else class="text-lg font-bold text-cyan-600">{{ patient.full_name?.charAt(0) }}</span>
                    </div>
                    <div class="flex-1 min-w-0">
                        <p class="text-sm font-semibold text-gray-800 group-hover:text-cyan-700 transition-colors">{{ patient.full_name }}</p>
                    ...
                    <div class="flex items-center gap-2 flex-shrink-0">
                        <span class="px-3 py-1.5 text-xs font-semibold rounded-lg bg-cyan-50 text-cyan-600 group-hover:bg-cyan-600 group-hover:text-white transition-all">
                            {{ isRtl ? 'فتح المخطط' : 'Open Chart' }}
                        </span>
                        <svg class="w-4 h-4 text-gray-300 group-hover:text-cyan-500 transition-colors" :class="isRtl ? 'rotate-180' : ''" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7" /></svg>
                    </div>
                </button>
            </div>
        </div>
        ...
        <div v-if="!searchQuery && recentCharts.length === 0" class="dental-card-enter text-center py-16" style="animation-delay: 0.25s">
            <div class="w-20 h-20 mx-auto mb-4 rounded-2xl bg-gradient-to-br from-cyan-100 to-cyan-50 flex items-center justify-center">
