    ...
    <div>
                            ...
                            <div class="grid grid-cols-12 gap-2 items-start">
                                ...
                                <div class="col-span-6 lg:col-span-2 relative">
                                    <div class="relative">
                                        ...
                                        <button type="button" @click="togglePreset(`freq-c-${index}`)" class="absolute right-2 top-1/2 -translate-y-1/2 text-gray-400 hover:text-[#C4A265]" :aria-label="isRtl ? 'توسيع' : 'Expand'" :title="isRtl ? 'توسيع' : 'Expand'"><svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7" /></svg></button>
                                    </div>
                                ...
                                <div class="col-span-6 lg:col-span-2 relative">
                                    <div class="relative">
                                        ...
                                        <button type="button" @click="togglePreset(`dur-c-${index}`)" class="absolute right-2 top-1/2 -translate-y-1/2 text-gray-400 hover:text-[#C4A265]" :aria-label="isRtl ? 'توسيع' : 'Expand'" :title="isRtl ? 'توسيع' : 'Expand'"><svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7" /></svg></button>
                                    </div>
                        ...
                        <tr v-for="rx in prescriptions.data" :key="rx.id" class="hover:bg-[#FDF8F0]/50 transition-colors group">
                            ...
                            <td class="px-3 sm:px-5 py-3.5 text-right">
                                <div class="flex flex-wrap items-center justify-end gap-1 opacity-70 group-hover:opacity-100 transition-opacity">
                                    ...
                                    <button @click="downloadPdf(rx.id)" class="p-1.5 text-gray-500 hover:text-[#1B365D] hover:bg-slate-50 rounded-lg transition-colors" :title="isRtl ? 'تحميل PDF' : 'Download PDF'"><svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4" /></svg></button>
        ...
        <Teleport to="body">
            <Transition enter-active-class="transition-all duration-200" enter-from-class="opacity-0" enter-to-class="opacity-100" leave-active-class="transition-all duration-150" leave-from-class="opacity-100" leave-to-class="opacity-0">
                <div v-if="viewingPrescription" v-focus-trap="() => (viewingPrescription = null)" role="dialog" aria-modal="true" class="fixed inset-0 z-[9999] flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm" @click.self="viewingPrescription = null">
                    <div class="bg-white rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto">
                        <div class="sticky top-0 bg-white border-b border-gray-100 px-4 sm:px-6 py-4 flex items-center justify-between z-10">
                            ...
                            <div class="flex flex-wrap items-center gap-2">
                                ...
                                <button @click="downloadPdf(viewingPrescription.id)" class="inline-flex items-center gap-1.5 px-3 py-2 text-xs font-semibold text-[#1B365D] bg-slate-50 hover:bg-slate-100 rounded-lg transition-colors">
                                    <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4" /></svg>
                                    {{ isRtl ? 'تحميل' : 'Download' }}
                                </button>
                                <button @click="viewingPrescription = null" class="w-8 h-8 rounded-lg flex items-center justify-center text-gray-400 hover:text-gray-600 hover:bg-gray-100" :aria-label="isRtl ? 'إغلاق' : 'Close'" :title="isRtl ? 'إغلاق' : 'Close'"><svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12" /></svg></button>
                            </div>
                        </div>
                        <div class="p-4 sm:p-6 space-y-5">
                            ...
                            <div v-if="viewingPrescription.notes" class="p-4 bg-slate-50/50 border border-slate-100 rounded-xl">
                                <p class="text-[10px] font-bold text-[#1B365D] uppercase tracking-wider mb-1">{{ isRtl ? 'ملاحظات' : 'Notes' }}</p>
        ...
        <Teleport to="body">
            <Transition enter-active-class="transition-all duration-200" enter-from-class="opacity-0" enter-to-class="opacity-100" leave-active-class="transition-all duration-150" leave-from-class="opacity-100" leave-to-class="opacity-0">
                <div v-if="editingPrescription" v-focus-trap="() => (editingPrescription = null)" role="dialog" aria-modal="true" class="fixed inset-0 z-[9999] flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm" @click.self="editingPrescription = null">
        ...
        <Teleport to="body">
            <Transition enter-active-class="transition-all duration-200" enter-from-class="opacity-0" enter-to-class="opacity-100" leave-active-class="transition-all duration-150" leave-from-class="opacity-100" leave-to-class="opacity-0">
                <div v-if="showDeleteConfirm" v-focus-trap="() => (showDeleteConfirm = null)" role="dialog" aria-modal="true" class="fixed inset-0 z-[9999] flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm" @click.self="showDeleteConfirm = null">
