...
const revenueMethodChartData = computed(() => {
    ...
    return props.revenueByMethod.map(d => ({ label: d.label, value: d.value }));
});

const expenseCategoryChartData = computed(() => {
    if (!props.expensesByCategory?.length) return [];
    return props.expensesByCategory.map(d => ({ label: d.label, value: d.value }));
});

/* ── Totals for data tables ────────────────────────── */
...
/* ── Revenue % per method for inline bars ──────────── */
...
const kpiCards = computed(() => [
    ...
    },
    {
        ...
        badgeLabel: `${expenseRatio.value}% of revenue`,
    ...
    },
    {
        ...
        icon: 'net',
        gradient: props.netIncome >= 0 ? 'from-blue-500 to-indigo-600' : 'from-red-500 to-rose-600',
        ...
        badgeLabel: `${profitMargin.value}% margin`,
...
/* ── Colors for donut method slices ────────────────── */
        ...
        <div class="space-y-8">
            ...
            >
                <div class="flex items-center gap-3">
                    <Link href="/admin/reports" class="w-10 h-10 rounded-xl bg-white border border-gray-200 shadow-sm flex items-center justify-center text-gray-400 hover:text-[#C4A265] hover:border-[#C4A265]/30 hover:shadow-md transition-all duration-300 group">
                        <svg class="w-5 h-5 group-hover:-translate-x-0.5 transition-transform duration-300" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7" /></svg>
                    </Link>
                    <div class="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <p class="text-sm text-gray-500">Revenue, expenses and financial analytics</p>
                        </div>
                    </div>
                </div>
                ...
                <div class="flex items-center gap-2 flex-wrap">
                    <div v-if="avgDailyRevenue" class="px-3 py-1.5 rounded-lg bg-white border border-gray-100 shadow-sm">
                        <span class="text-[10px] font-medium text-gray-400 uppercase tracking-wider block">Avg Daily</span>
                        <span class="text-sm font-bold text-gray-900">{{ formatCurrency(avgDailyRevenue) }}</span>
                    </div>
                    <div v-if="peakDay" class="px-3 py-1.5 rounded-lg bg-white border border-gray-100 shadow-sm">
                        <span class="text-[10px] font-medium text-gray-400 uppercase tracking-wider block">Peak Day</span>
            ...
            >
                ...
                <button
                    ...
                    class="ml-auto px-3 py-2 text-xs font-medium text-gray-400 hover:text-red-500 rounded-lg hover:bg-red-50 transition-all duration-200"
                >
                    Clear
                </button>
            </div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-5">
                ...
                >
                    ...
                    <div :class="`absolute top-0 left-0 right-0 h-[3px] bg-gradient-to-r ${card.accentColor} opacity-0 group-hover:opacity-100 transition-opacity duration-500`"></div>
                    ...
                    <div class="relative flex items-start justify-between">
                        <div class="flex-1">
                            ...
                            <p class="text-[26px] font-bold text-gray-900 mt-2 leading-none">{{ card.value }}</p>
                            <div class="mt-3">
                                <span
                        ...
                        <!-- Icon -->
                        <div :class="card.bg" class="w-14 h-14 rounded-2xl flex items-center justify-center flex-shrink-0 transition-all duration-300 group-hover:scale-110 group-hover:shadow-lg">
                            <svg v-if="card.icon === 'revenue'" :class="card.iconColor" class="w-7 h-7" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M2.25 18.75a60.07 60.07 0 0115.797 2.101c.727.198 1.453-.342 1.453-1.096V18.75M3.75 4.5v.75A.75.75 0 013 6h-.75m0 0v-.375c0-.621.504-1.125 1.125-1.125H20.25M2.25 6v9m18-10.5v.75c0 .414.336.75.75.75h.75m-1.5-1.5h.375c.621 0 1.125.504 1.125 1.125v9.75c0 .621-.504 1.125-1.125 1.125h-.375m1.5-1.5H21a.75.75 0 00-.75.75v.75m0 0H3.75m0 0h-.375a1.125 1.125 0 01-1.125-1.125V15m1.5 1.5v-.75A.75.75 0 003 15h-.75M15 10.5a3 3 0 11-6 0 3 3 0 016 0zm3 0h.008v.008H18V10.5zm-12 0h.008v.008H6V10.5z" />
                            </svg>
                            <svg v-else-if="card.icon === 'expenses'" :class="card.iconColor" class="w-7 h-7" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M13 17h8m0 0V9m0 8l-8-8-4 4-6-6" />
                            </svg>
                            <svg v-else-if="card.icon === 'net'" :class="card.iconColor" class="w-7 h-7" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M12 6v12m-3-2.818l.879.659c1.171.879 3.07.879 4.242 0 1.172-.879 1.172-2.303 0-3.182C13.536 12.219 12.768 12 12 12c-.725 0-1.45-.22-2.003-.659-1.106-.879-1.106-2.303 0-3.182s2.9-.879 4.006 0l.415.33M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
                            </svg>
                            <svg v-else-if="card.icon === 'unpaid'" :class="card.iconColor" class="w-7 h-7" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            >
                <div class="flex items-center justify-between mb-4">
                    <div>
                        <h2 class="text-[15px] font-semibold text-gray-900">Revenue vs Expenses</h2>
                        <p class="text-xs text-gray-400 mt-0.5">Financial balance overview</p>
                    </div>
                    <div class="flex items-center gap-4">
                        <div class="flex items-center gap-1.5">
                            <div class="w-3 h-3 rounded-full bg-emerald-500"></div>
                            <span class="text-[11px] text-gray-500">Revenue</span>
                        </div>
                        <div class="flex items-center gap-1.5">
                            <div class="w-3 h-3 rounded-full bg-rose-500"></div>
                            <span class="text-[11px] text-gray-500">Expenses</span>
                        </div>
                    </div>
                </div>
                ...
                <div class="relative h-10 rounded-xl overflow-hidden bg-gray-100">
                    <div
                        ...
                        class="absolute inset-y-0 left-0 bg-gradient-to-r from-emerald-500 to-emerald-400 transition-all duration-1000 ease-out rounded-xl"
                    ...
                    <div
                        v-if="totalRevenue > 0 || totalExpenses > 0"
                        class="absolute inset-y-0 right-0 bg-gradient-to-r from-rose-400 to-rose-500 transition-all duration-1000 ease-out rounded-xl"
            ...
            >
                <div class="flex items-center justify-between mb-5">
                    <div>
                        ...
                        <p class="text-xs text-gray-400 mt-0.5">Revenue trend for the selected period</p>
                    </div>
                    <div v-if="dailyRevenue?.length" class="flex items-center gap-3">
                        <div class="text-right">
                            <p class="text-[10px] font-medium text-gray-400 uppercase tracking-wider">Days</p>
                            <p class="text-sm font-bold text-gray-900">{{ dailyRevenue.length }}</p>
                        </div>
                        <div class="w-px h-8 bg-gray-200"></div>
                        <div class="text-right">
                            <p class="text-[10px] font-medium text-gray-400 uppercase tracking-wider">Average</p>
                            <p class="text-sm font-bold text-[#C4A265]">{{ formatCurrency(avgDailyRevenue) }}</p>
                        </div>
                    </div>
                </div>
                <div v-if="dailyRevenueChartData.length" class="overflow-x-auto">
                    <BarChart :data="dailyRevenueChartData" color="#C4A265" :valueSuffix="' ' + currencyCode" />
                </div>
            ...
            <!-- ── Two-column: Revenue by Method & Expenses by Category ── -->
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                >
                    <div class="p-6 pb-4">
                        <div class="flex items-center justify-between">
                            <div>
                                ...
                                <p class="text-xs text-gray-400 mt-0.5">Breakdown of payment channels</p>
                            </div>
                    ...
                    <div v-if="revenueMethodChartData.length" class="px-6 pb-4">
                        <DonutChart :data="revenueMethodChartData" />
                    </div>

                    <!-- Data rows with inline bars -->
                    <div class="border-t border-gray-100">
                        <div v-if="revenueByMethod?.length" class="divide-y divide-gray-50">
                            <div
                                v-for="(item, i) in revenueByMethod"
                                :key="item.label"
                                class="px-6 py-3.5 hover:bg-gray-50/50 transition-colors duration-200"
                            >
                                <div class="flex items-center justify-between mb-1.5">
                                    <div class="flex items-center gap-2.5">
                                        ...
                                        <span class="text-sm font-medium text-gray-800 capitalize">{{ item.label || 'Unknown' }}</span>
                                    </div>
                                    <div class="flex items-center gap-3">
                                        <span class="text-[11px] text-gray-400">{{ item.count || 0 }} txns</span>
                        ...
                        <div v-else class="px-6 py-10 text-center text-sm text-gray-400">No revenue data found.</div>

                        <!-- Total footer -->
                        <div v-if="revenueByMethod?.length" class="px-6 py-3.5 bg-gray-50/80 border-t border-gray-100">
                            <div class="flex items-center justify-between">
                                <span class="text-sm font-bold text-gray-700">Total</span>
                                <div class="flex items-center gap-3">
                                    <span class="text-[11px] text-gray-400">{{ revenueByMethod.reduce((s, d) => s + (d.count || 0), 0) }} txns</span>
                ...
                >
                    <div class="p-6 pb-4">
                        <div class="flex items-center justify-between">
                            <div>
                                ...
                                <p class="text-xs text-gray-400 mt-0.5">Spending breakdown by category</p>
                            </div>
                    ...
                    <div v-if="expenseCategoryChartData.length" class="px-6 pb-4">
                        <DonutChart :data="expenseCategoryChartData" />
                    </div>
                    ...
                    <div class="border-t border-gray-100">
                            ...
                            >
                                <div class="flex items-center justify-between mb-1.5">
                                    <div class="flex items-center gap-2.5">
                                        ...
                                        <span class="text-sm font-medium text-gray-800 capitalize">{{ item.label || 'Uncategorized' }}</span>
                                    </div>
                                    <div class="flex items-center gap-3">
                                        <span class="text-[11px] text-gray-400">{{ item.count || 0 }} items</span>
                        ...
                        <div v-else class="px-6 py-10 text-center text-sm text-gray-400">No expense data found.</div>

                        <!-- Total footer -->
                        <div v-if="expensesByCategory?.length" class="px-6 py-3.5 bg-gray-50/80 border-t border-gray-100">
                            <div class="flex items-center justify-between">
                                <span class="text-sm font-bold text-gray-700">Total</span>
                                <div class="flex items-center gap-3">
                                    <span class="text-[11px] text-gray-400">{{ expensesByCategory.reduce((s, d) => s + (d.count || 0), 0) }} items</span>
            ...
            >
                ...
                <div class="relative">
                    <div class="flex items-center gap-2 mb-5">
                        ...
                        <h2 class="text-[15px] font-semibold">Financial Summary</h2>
                    </div>

                    <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                        <div class="bg-white/5 backdrop-blur rounded-xl p-4 border border-white/10">
                            <p class="text-[10px] font-medium text-gray-400 uppercase tracking-wider mb-1">Revenue</p>
                            <p class="text-lg font-bold text-emerald-400">{{ formatCurrency(totalRevenue) }}</p>
                        </div>
                        <div class="bg-white/5 backdrop-blur rounded-xl p-4 border border-white/10">
                            <p class="text-[10px] font-medium text-gray-400 uppercase tracking-wider mb-1">Expenses</p>
                        ...
                        <div class="bg-white/5 backdrop-blur rounded-xl p-4 border border-white/10">
                            <p class="text-[10px] font-medium text-gray-400 uppercase tracking-wider mb-1">Unpaid</p>
                    ...
                    <div class="mt-4 flex flex-wrap gap-3">
                        <div class="px-3 py-1.5 rounded-lg bg-white/5 border border-white/10 text-[11px]">
                            ...
                            <span class="ml-1 font-bold" :class="profitMargin >= 30 ? 'text-emerald-400' : profitMargin >= 0 ? 'text-amber-400' : 'text-red-400'">{{ profitMargin }}%</span>
                        </div>
                        <div class="px-3 py-1.5 rounded-lg bg-white/5 border border-white/10 text-[11px]">
                            <span class="text-gray-400">Expense Ratio:</span>
                            <span class="ml-1 font-bold" :class="expenseRatio <= 50 ? 'text-emerald-400' : expenseRatio <= 70 ? 'text-amber-400' : 'text-red-400'">{{ expenseRatio }}%</span>
                        </div>
                        <div v-if="revenueByMethod?.length" class="px-3 py-1.5 rounded-lg bg-white/5 border border-white/10 text-[11px]">
                            ...
                            <span class="ml-1 font-bold text-[#C4A265]">{{ revenueByMethod.length }}</span>
                        </div>
                        <div v-if="expensesByCategory?.length" class="px-3 py-1.5 rounded-lg bg-white/5 border border-white/10 text-[11px]">
                            <span class="text-gray-400">Expense Categories:</span>
                            <span class="ml-1 font-bold text-[#C4A265]">{{ expensesByCategory.length }}</span>
                        </div>
                    </div>
                </div>
            </div>
