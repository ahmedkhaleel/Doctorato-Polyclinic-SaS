...
const props = defineProps({ photos: Object, filters: Object, categories: Array, patients: Array });

const patientId = ref(props.filters?.patient_id || '');
const category = ref(props.filters?.category || '');
watch([patientId, category], () => {
    router.get('/admin/derma/gallery', {
        patient_id: patientId.value || undefined, category: category.value || undefined,
    }, { preserveState: true, preserveScroll: true });
});

const showUpload = ref(false);
const form = useForm({
    patient_id: '', category: 'before', body_area: '', taken_at: '',
    image: null, notes: '',
});
function submit() {
...
<template>
    <div class="space-y-6 pb-10">
        ...
        <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
            <div v-for="p in photos.data" :key="p.id" class="bg-white rounded-xl border border-gray-100 overflow-hidden group">
                ...
                <div class="p-3">
                    ...
                    <div class="text-[10px] text-pink-600 uppercase mt-1">{{ p.category }}</div>
            ...
            <div class="bg-white rounded-2xl w-full max-w-md p-6">
                ...
                <form @submit.prevent="submit" class="space-y-3">
                    ...
                    <div class="flex justify-end gap-2 pt-2">
                        ...
                        <button :disabled="form.processing" class="px-5 py-2 rounded-lg bg-pink-600 text-white text-sm font-semibold">{{ t('Upload', 'رفع') }}</button>
                    </div>
                </form>
            </div>
        </div>
