...
const priorityConfig = computed(() => ({
    1: { label: isRtl.value ? 'ساخن' : 'Hot', emoji: '\uD83D\uDD25', bg: '#fef2f2', text: '#dc2626', border: '#fecaca' },
    2: { label: isRtl.value ? 'دافئ' : 'Warm', emoji: '\u2600\uFE0F', bg: '#fffbeb', text: '#d97706', border: '#fde68a' },
    3: { label: isRtl.value ? 'بارد' : 'Cold', emoji: '\u2744\uFE0F', bg: '#eff6ff', text: '#2563eb', border: '#bfdbfe' },
}));
                ...
                <div class="flex flex-col lg:flex-row gap-3">
                    ...
                    <div class="relative min-w-[160px]">
                        <select v-model="priorityFilter"
                            ...
                            <option value="2">{{ isRtl ? 'دافئ' : 'Warm' }} \u2600\uFE0F</option>
                            <option value="3">{{ isRtl ? 'بارد' : 'Cold' }} \u2744\uFE0F</option>
                        </select>
                        <div class="absolute inset-y-0 ltr:right-0 rtl:left-0 ltr:pr-3 rtl:pl-3 flex items-center pointer-events-none">
                            <svg class="w-4 h-4 text-gray-400" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" d="M19 9l-7 7-7-7"/>
                ...
                <div v-for="(lead, idx) in leads.data" :key="lead.id"
                    ...
                    <div class="flex items-start gap-3 mb-4">
                        ...
                        <div class="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity duration-200">
                            ...
                            <Link :href="`/secretary/crm/leads/${lead.id}`"
                               class="w-8 h-8 rounded-lg flex items-center justify-center text-gray-400 hover:text-teal-600 hover:bg-teal-50 transition-all" :title="isRtl ? 'عرض' : 'View'">
                                <svg class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
