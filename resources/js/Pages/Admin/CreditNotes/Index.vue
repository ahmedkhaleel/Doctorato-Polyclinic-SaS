        ...
        <div class="bg-white rounded-2xl border border-gray-100 overflow-hidden">
            <table class="w-full text-sm">
                ...
                <tbody class="divide-y divide-gray-50">
                    <tr v-for="(cn, i) in creditNotes.data" :key="cn.id" class="lst-row hover:bg-gray-50/50" :style="{ '--row-i': i }">
                        ...
                        <td class="px-4 py-3 text-center">
                            <button v-if="allowedTransitions[cn.status]?.length" @click="openStatusChange(cn)" class="p-1.5 text-gray-400 hover:text-[#1B365D] hover:bg-slate-50 rounded-lg transition" :aria-label="isRtl ? 'تغيير الحالة' : 'Change status'" :title="isRtl ? 'تغيير الحالة' : 'Change status'">
