...
<template>
    <div class="p-6 max-w-7xl mx-auto">
        <!-- Header -->
        <div class="mb-6">
            <h1 class="text-2xl font-bold text-gray-800">{{ isRtl ? 'رضا المرضى' : 'Patient Satisfaction' }}</h1>
        ...
        <div class="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-7 gap-4 mb-6">
            ...
            <div class="bg-white rounded-2xl border border-gray-100 p-4 text-center">
                ...
                <p class="text-2xl font-bold text-amber-500 mt-1">{{ stats.avg_overall }} <span class="text-sm">/ 5</span></p>
            </div>
            <div class="bg-white rounded-2xl border border-gray-100 p-4 text-center">
                <p class="text-xs text-gray-400">{{ isRtl ? 'تقييم الطبيب' : 'Doctor Rating' }}</p>
                <p class="text-2xl font-bold text-cyan-600 mt-1">{{ stats.avg_doctor }} <span class="text-sm">/ 5</span></p>
            </div>
            <div class="bg-white rounded-2xl border border-gray-100 p-4 text-center">
                <p class="text-xs text-gray-400">{{ isRtl ? 'تقييم الموظفين' : 'Staff Rating' }}</p>
                <p class="text-2xl font-bold text-purple-600 mt-1">{{ stats.avg_staff }} <span class="text-sm">/ 5</span></p>
            </div>
            <div class="bg-white rounded-2xl border border-gray-100 p-4 text-center">
                <p class="text-xs text-gray-400">{{ isRtl ? 'وقت الانتظار' : 'Waiting Rating' }}</p>
                <p class="text-2xl font-bold mt-1" :class="stats.avg_waiting >= 3.5 ? 'text-green-600' : 'text-red-600'">{{ stats.avg_waiting }} <span class="text-sm">/ 5</span></p>
            </div>
            <div class="bg-white rounded-2xl border border-gray-100 p-4 text-center">
                <p class="text-xs text-gray-400">{{ isRtl ? 'نسبة التوصية' : 'Recommend %' }}</p>
                <p class="text-2xl font-bold text-green-600 mt-1">{{ stats.recommend_rate }}%</p>
            </div>
            <div class="bg-white rounded-2xl border border-gray-100 p-4 text-center">
                <p class="text-xs text-gray-400">NPS</p>
                <p class="text-2xl font-bold mt-1" :class="npsColor(stats.nps)">{{ stats.nps }}</p>
            </div>
        </div>
                ...
                <div class="space-y-3">
                    <div v-for="i in [5,4,3,2,1]" :key="i" class="flex items-center gap-3">
                        ...
                        <div class="flex-1 bg-gray-100 rounded-full h-4 overflow-hidden">
                            <div class="h-full rounded-full transition-all" :class="i >= 4 ? 'bg-green-500' : i === 3 ? 'bg-yellow-500' : 'bg-red-500'"
                ...
                <div v-if="doctorRankings.length > 0" class="space-y-3">
                    <div v-for="(ranking, idx) in doctorRankings" :key="ranking.doctor_id" class="flex items-center gap-3">
                        <span class="w-6 h-6 rounded-full flex items-center justify-center text-xs font-bold"
                            :class="idx === 0 ? 'bg-amber-100 text-amber-700' : idx === 1 ? 'bg-gray-100 text-gray-600' : idx === 2 ? 'bg-orange-100 text-orange-600' : 'bg-gray-50 text-gray-400'">
                            {{ idx + 1 }}
                        </span>
        ...
        <div class="flex flex-wrap gap-3 mb-4">
            <select v-model="doctorFilter" @change="applyFilters" class="px-4 py-2 border border-gray-200 rounded-xl text-sm focus:ring-cyan-500 focus:border-cyan-500">
            ...
            <select v-model="ratingFilter" @change="applyFilters" class="px-4 py-2 border border-gray-200 rounded-xl text-sm focus:ring-cyan-500 focus:border-cyan-500">
            ...
            <div v-for="fb in feedbacks.data" :key="fb.id" class="bg-white rounded-2xl border border-gray-100 p-5 hover:shadow-sm transition">
                <div class="flex items-start justify-between mb-3">
                    <div class="flex items-center gap-3">
                        <div class="w-10 h-10 rounded-full bg-cyan-50 flex items-center justify-center text-cyan-600 font-bold text-sm">
                ...
                <div class="flex flex-wrap gap-4 text-xs text-gray-500">
                    ...
                    <span v-if="fb.nps_score !== null" class="px-2 py-0.5 rounded-full" :class="fb.nps_category?.color === 'green' ? 'bg-green-50 text-green-600' : fb.nps_category?.color === 'yellow' ? 'bg-yellow-50 text-yellow-600' : 'bg-red-50 text-red-600'">
                        NPS: {{ fb.nps_score }} ({{ isRtl ? fb.nps_category?.ar : fb.nps_category?.en }})
                    </span>
                    <span v-if="fb.would_recommend === true" class="text-green-600">{{ isRtl ? '✓ يوصي' : '✓ Recommends' }}</span>
