...
const typeColors = {
    ...
    usage: 'bg-slate-100 text-[#1B365D]',
    adjustment: 'bg-amber-100 text-amber-800',
    return: 'bg-slate-100 text-[#1B365D]',
};
    ...
    <AdminLayout :title="`${$t('a_transactions')} - ${supply.name_en}`">
        <div class="space-y-6">
            <div class="flex items-center justify-between">
                <div>
                    <h1 class="text-xl md:text-2xl font-bold text-gray-800">{{ supply.name_en }}</h1>
                    <p class="text-sm text-gray-500 mt-1">{{ $t('a_transaction_history') }}</p>
                </div>
                <div class="flex items-center space-x-3 rtl:space-x-reverse">
                    <span class="text-sm font-mono px-3 py-1 rounded-full bg-amber-50" style="color: #6366F1;">{{ supply.sku }}</span>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-4 gap-4">
                <div class="bg-white rounded-lg shadow-sm p-4">
                    ...
                    <p class="text-xl md:text-2xl font-bold" :class="isLowStock() ? 'text-red-600' : 'text-gray-900'">{{ supply.quantity }} {{ supply.unit || '' }}</p>
                </div>
                <div class="bg-white rounded-lg shadow-sm p-4">
                    <p class="text-xs text-gray-500 uppercase">{{ $t('a_min_quantity') }}</p>
                    <p class="text-xl md:text-2xl font-bold text-gray-900">{{ supply.min_quantity }}</p>
                </div>
                <div class="bg-white rounded-lg shadow-sm p-4">
                    <p class="text-xs text-gray-500 uppercase">{{ $t('a_status') }}</p>
                    <span
                        :class="isLowStock() ? 'bg-red-100 text-red-800' : 'bg-emerald-100 text-emerald-800'"
                ...
                <div v-if="showForm" class="p-6">
                    <form @submit.prevent="submit" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                        <div>
                            ...
                            <select v-model="form.transaction_type" class="doctorato-input w-full px-4 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-[#1B365D]/20 focus:border-transparent">
                        ...
                        <div>
                            ...
                            <input v-model="form.quantity" type="number" class="doctorato-input w-full px-4 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-[#1B365D]/20 focus:border-transparent" :placeholder="$t('a_use_negative_reduction')" />
                        ...
                        <div>
                            ...
                            <input v-model="form.unit_cost" type="number" step="0.01" min="0" class="doctorato-input w-full px-4 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-[#1B365D]/20 focus:border-transparent" placeholder="0.00" />
                        ...
                        <div>
                            ...
                            <input v-model="form.notes" type="text" class="doctorato-input w-full px-4 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-[#1B365D]/20 focus:border-transparent" :placeholder="$t('a_optional_notes')" />
                        </div>
                    ...
                    <table class="min-w-full divide-y divide-gray-200">
                        <thead class="bg-gray-50">
                            <tr>
                                ...
                                <th class="px-4 md:px-6 py-3 ltr:text-left rtl:text-right text-xs font-medium text-gray-500 uppercase tracking-wider">{{ $t('a_quantity') }}</th>
                                <th class="px-4 md:px-6 py-3 ltr:text-left rtl:text-right text-xs font-medium text-gray-500 uppercase tracking-wider">{{ $t('a_unit_cost') }}</th>
                                <th class="px-4 md:px-6 py-3 ltr:text-left rtl:text-right text-xs font-medium text-gray-500 uppercase tracking-wider">{{ $t('a_visit') }}</th>
                                <th class="px-4 md:px-6 py-3 ltr:text-left rtl:text-right text-xs font-medium text-gray-500 uppercase tracking-wider">{{ $t('a_notes') }}</th>
                                <th class="px-4 md:px-6 py-3 ltr:text-left rtl:text-right text-xs font-medium text-gray-500 uppercase tracking-wider">{{ $t('a_by') }}</th>
                            </tr>
                        </thead>
                        <tbody class="bg-white divide-y divide-gray-200">
                            <tr v-for="(txn, i) in transactions.data" :key="txn.id" class="lst-row hover:bg-gray-50" :style="{ '--row-i': i }">
                                <td class="px-4 md:px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ formatDate(txn.created_at) }}</td>
                                ...
                                <td class="px-4 md:px-6 py-4 whitespace-nowrap text-sm font-semibold" :class="txn.quantity > 0 ? 'text-emerald-600' : 'text-red-600'">
                                    {{ txn.quantity > 0 ? '+' : '' }}{{ txn.quantity }}
                                </td>
                                <td class="px-4 md:px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ txn.unit_cost ? formatCurrency(txn.unit_cost) : '-' }}</td>
                                ...
                                <td class="px-4 md:px-6 py-4 text-sm text-gray-500 max-w-xs truncate">{{ txn.notes || '-' }}</td>
                                <td class="px-4 md:px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ txn.user?.name || '-' }}</td>
                            </tr>
                            <tr v-if="!transactions.data || transactions.data.length === 0">
                                <td colspan="7" class="px-4 md:px-6 py-8 text-center text-sm text-gray-500">{{ $t('a_no_transactions_found') }}</td>
                            </tr>
                        </tbody>
                    </table>
                </div>

                <div v-if="transactions.links && transactions.links.length > 3" class="px-4 md:px-6 py-3 border-t border-gray-200 flex items-center justify-between">
