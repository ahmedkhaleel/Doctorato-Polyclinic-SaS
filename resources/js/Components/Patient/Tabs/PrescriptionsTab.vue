            ...
            <div v-for="rx in prescriptions" :key="rx.id" class="bg-white rounded-xl border border-gray-100 overflow-hidden">
                <div class="px-5 py-3 border-b border-gray-50 flex items-center justify-between">
                    <div class="flex items-center gap-3">
                        <div class="w-9 h-9 rounded-lg bg-slate-50 text-[#1B365D] flex items-center justify-center flex-shrink-0">
                    ...
                    <div v-if="rx.items?.length" class="space-y-1.5">
                        ...
                        <div class="flex flex-wrap gap-1.5">
                            <span v-for="item in rx.items" :key="item.id"
                                class="inline-flex items-center px-2.5 py-1 rounded-lg bg-slate-50 text-[#1B365D] text-xs font-medium">
                                {{ item.medication_name || item.name || item.drug_name || (isRtl ? 'دواء' : 'Medication') }}
                                <span v-if="item.dosage" class="text-[#1B365D] ltr:ml-1 rtl:mr-1">· {{ item.dosage }}</span>
                            </span>
                        </div>
                    </div>
