...
const tabs = [
    { id: 'overview', label: 'Overview', key: 'a_overview' },
    { id: 'visits', label: 'Visits & Queue', key: 'a_visits_queue' },
    { id: 'patients', label: 'Patients', key: 'a_patients' },
    { id: 'prescriptions', label: 'Prescriptions', key: 'a_prescriptions' },
...
const visitTypeLabels = {
    consultation: 'Consultation', // translated in template
    session: 'Session',
    follow_up: 'Follow Up',
};
            ...
            <div class="bg-white rounded-lg shadow-sm p-6">
                <div class="flex items-start justify-between">
                    <div class="flex items-center ltr:space-x-4 rtl:space-x-reverse rtl:space-x-4">
                        ...
                        <div>
                            <h1 class="text-2xl font-bold text-gray-800">{{ $t('a_doctor') }} {{ $localized(doctor, 'name') }}</h1>
                            <p class="text-lg text-gray-500" :dir="isRtl ? 'ltr' : 'rtl'">{{ locale === 'ar' ? doctor.name_en : 'د. ' + doctor.name_ar }}</p>
                            <p class="text-sm text-gray-500 mt-0.5">{{ $localized(doctor, 'specialization') }}</p>
                            <div class="flex items-center ltr:space-x-3 rtl:space-x-reverse rtl:space-x-3 mt-2">
                                <StatusBadge :status="doctor.status" />
                                <span v-if="doctor.consultation_fee" class="text-sm font-medium" style="color: #C4A265;">
                                    {{ $t('a_fee') }}: {{ formatCurrency(doctor.consultation_fee) }}
                                </span>
                            </div>
                        </div>
                    </div>
                ...
                <div class="p-6">
                    ...
                    <div v-if="activeTab === 'overview'" class="space-y-6">
                        ...
                        <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4">
                            <div class="bg-gray-50 rounded-lg p-4">
                                <p class="text-xs text-gray-500 uppercase tracking-wider">{{ $t('a_total_visits') }}</p>
                                <p class="text-2xl font-bold text-gray-800 mt-1">{{ performanceStats.total_visits }}</p>
                            </div>
                            <div class="bg-green-50 rounded-lg p-4">
                                <p class="text-xs text-gray-500 uppercase tracking-wider">{{ $t('a_completed') }}</p>
                        ...
                        <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                            <div class="bg-gray-50 rounded-lg p-5 space-y-3">
                                <h3 class="text-sm font-semibold text-gray-700 uppercase tracking-wider">{{ $t('a_personal_information') }}</h3>
                                <dl class="space-y-2 text-sm">
                                    <div class="flex justify-between"><dt class="text-gray-500">{{ $t('a_phone') }}</dt><dd class="text-gray-900">{{ doctor.phone || '-' }}</dd></div>
                                    <div class="flex justify-between"><dt class="text-gray-500">{{ $t('a_email') }}</dt><dd class="text-gray-900">{{ doctor.email || '-' }}</dd></div>
                                    <div class="flex justify-between"><dt class="text-gray-500">{{ $t('a_specialization_en') }}</dt><dd class="text-gray-900">{{ doctor.specialization_en || '-' }}</dd></div>
                                    <div class="flex justify-between"><dt class="text-gray-500">{{ $t('a_specialization_ar') }}</dt><dd class="text-gray-900" dir="rtl">{{ doctor.specialization_ar || '-' }}</dd></div>
                                </dl>
                                <div v-if="doctor.qualifications_en" class="pt-2 border-t border-gray-200">
                                    <p class="text-xs text-gray-500 uppercase tracking-wider mb-1">{{ $t('a_qualifications') }}</p>
                                    <p class="text-sm text-gray-800">{{ doctor.qualifications_en }}</p>
                                </div>
                                <div v-if="doctor.bio_en" class="pt-2 border-t border-gray-200">
                                    <p class="text-xs text-gray-500 uppercase tracking-wider mb-1">{{ $t('a_bio') }}</p>
                                    <div class="text-sm text-gray-800 prose prose-sm max-w-none" v-html="sanitizeHtml(doctor.bio_en)"></div>
                                </div>
                            </div>
                            <div class="space-y-4">
                                <div class="bg-gray-50 rounded-lg p-5 space-y-3">
                                    <h3 class="text-sm font-semibold text-gray-700 uppercase tracking-wider">{{ $t('a_clinic_details') }}</h3>
                                    <dl class="space-y-2 text-sm">
                                        <div class="flex justify-between"><dt class="text-gray-500">{{ $t('a_default_commission') }}</dt><dd class="font-medium" style="color: #C4A265;">{{ doctor.default_commission_percentage || 0 }}%</dd></div>
                                        <div class="flex justify-between"><dt class="text-gray-500">{{ $t('a_consultation_fee') }}</dt><dd class="text-gray-900">{{ formatCurrency(doctor.consultation_fee) }}</dd></div>
                    ...
                    <div v-if="activeTab === 'visits'" class="space-y-6">
                        ...
                        <div v-if="todayQueue && todayQueue.length > 0">
                            <h3 class="text-sm font-semibold text-gray-700 uppercase tracking-wider mb-3">{{ $t('a_todays_queue') }}</h3>
                            <div class="bg-gray-50 rounded-lg divide-y divide-gray-200">
                                <div v-for="visit in todayQueue" :key="visit.id" class="px-4 py-3 flex items-center justify-between">
                                    <div class="flex items-center ltr:space-x-3 rtl:space-x-reverse rtl:space-x-3">
                                        <StatusBadge :status="visit.status" type="visit" />
                                        <Link :href="`/admin/visits/${visit.id}`" class="text-sm font-medium text-gray-900 hover:underline">
                                            {{ visit.patient?.full_name }}
                                        </Link>
                                        <span class="text-xs text-gray-500">{{ visit.service ? $localized(visit.service, 'name') : '-' }}</span>
                                    </div>
                                    <Link :href="`/admin/visits/${visit.id}`" class="text-xs font-medium hover:underline" style="color: #C4A265;">{{ $t('a_view') }}</Link>
                                </div>
                            </div>
                        </div>
                        ...
                        <!-- All Visits Table -->
                        <div>
                            <h3 class="text-sm font-semibold text-gray-700 uppercase tracking-wider mb-3">{{ $t('a_all_visits') }}</h3>
                            <div class="overflow-x-auto">
                                <table class="min-w-full divide-y divide-gray-200">
                                    <thead class="bg-gray-50">
                                        <tr>
                                            <th class="px-4 py-3 ltr:text-left rtl:text-right text-xs font-medium text-gray-500 uppercase">{{ $t('a_date') }}</th>
                                            <th class="px-4 py-3 ltr:text-left rtl:text-right text-xs font-medium text-gray-500 uppercase">{{ $t('a_patient') }}</th>
                                            <th class="px-4 py-3 ltr:text-left rtl:text-right text-xs font-medium text-gray-500 uppercase">{{ $t('a_type') }}</th>
                                            <th class="px-4 py-3 ltr:text-left rtl:text-right text-xs font-medium text-gray-500 uppercase">{{ $t('a_service') }}</th>
                                            <th class="px-4 py-3 ltr:text-left rtl:text-right text-xs font-medium text-gray-500 uppercase">{{ $t('a_status') }}</th>
                                            <th class="px-4 py-3 ltr:text-left rtl:text-right text-xs font-medium text-gray-500 uppercase">{{ $t('a_commission') }}</th>
                                            <th class="px-4 py-3 ltr:text-right rtl:text-left text-xs font-medium text-gray-500 uppercase">{{ $t('a_actions') }}</th>
                                        </tr>
                                    </thead>
                                    <tbody class="bg-white divide-y divide-gray-200">
                                        <tr v-for="visit in visits" :key="visit.id" class="hover:bg-gray-50">
                                            ...
                                            <td class="px-4 py-3 whitespace-nowrap">
                                                <Link v-if="can('patients.view')" :href="`/admin/patients/${visit.patient?.id}`" class="text-sm font-medium hover:underline" style="color: #C4A265;">{{ visit.patient?.full_name }}</Link>
                                            ...
                                            <td class="px-4 py-3 whitespace-nowrap text-sm text-gray-500">{{ visit.service ? $localized(visit.service, 'name') : '-' || '-' }}</td>
                                            ...
                                            <td class="px-4 py-3 whitespace-nowrap ltr:text-right rtl:text-left">
                                                <Link :href="`/admin/visits/${visit.id}`" class="text-sm font-medium hover:underline" style="color: #C4A265;">{{ $t('a_view') }}</Link>
                                            </td>
                                        </tr>
                                        <tr v-if="!visits || visits.length === 0">
                                            <td colspan="7" class="px-4 py-8 text-center text-sm text-gray-500">{{ $t('a_no_visits_found_dot') }}</td>
                                        </tr>
                                    </tbody>
                                </table>
                            </div>
                        ...
                        <div class="overflow-x-auto">
                            <table class="min-w-full divide-y divide-gray-200">
                                <thead class="bg-gray-50">
                                    <tr>
                                        <th class="px-4 py-3 ltr:text-left rtl:text-right text-xs font-medium text-gray-500 uppercase">{{ $t('a_patient') }}</th>
                                        <th class="px-4 py-3 ltr:text-left rtl:text-right text-xs font-medium text-gray-500 uppercase">{{ $t('a_phone') }}</th>
                                        <th class="px-4 py-3 ltr:text-left rtl:text-right text-xs font-medium text-gray-500 uppercase">{{ $t('a_file_number') }}</th>
                                        <th class="px-4 py-3 ltr:text-left rtl:text-right text-xs font-medium text-gray-500 uppercase">{{ $t('a_visits') }}</th>
                                        <th class="px-4 py-3 ltr:text-left rtl:text-right text-xs font-medium text-gray-500 uppercase">{{ $t('a_last_visit') }}</th>
                                        <th class="px-4 py-3 ltr:text-right rtl:text-left text-xs font-medium text-gray-500 uppercase">{{ $t('a_actions') }}</th>
                                    </tr>
                                </thead>
                                <tbody class="bg-white divide-y divide-gray-200">
                                    <tr v-for="patient in patients" :key="patient.id" class="hover:bg-gray-50">
                                        <td class="px-4 py-3 whitespace-nowrap">
                                            <Link v-if="can('patients.view')" :href="`/admin/patients/${patient.id}`" class="text-sm font-medium hover:underline" style="color: #C4A265;">{{ patient.full_name }}</Link>
                                        ...
                                        <td class="px-4 py-3 whitespace-nowrap ltr:text-right rtl:text-left">
                                            <Link v-if="can('patients.view')" :href="`/admin/patients/${patient.id}`" class="text-sm font-medium hover:underline" style="color: #C4A265;">{{ $t('a_view_profile_link') }}</Link>
                                        </td>
                                    </tr>
                                    <tr v-if="!patients || patients.length === 0">
                                        <td colspan="6" class="px-4 py-8 text-center text-sm text-gray-500">{{ $t('a_no_patients_found') }}</td>
                                    </tr>
                                </tbody>
                            </table>
                        </div>
                    ...
                    <div v-if="activeTab === 'prescriptions'" class="space-y-4">
                        ...
                        <div v-for="rx in prescriptions" :key="rx.id" class="border rounded-lg overflow-hidden">
                            <div class="flex justify-between items-start p-4 border-b bg-gray-50">
                                <div>
                                    <div class="flex items-center ltr:space-x-3 rtl:space-x-reverse rtl:space-x-3">
                                        <Link v-if="can('patients.view') && rx.patient" :href="`/admin/patients/${rx.patient.id}`" class="text-sm font-medium hover:underline" style="color: #C4A265;">{{ rx.patient?.full_name }}</Link>
                                ...
                                <div class="flex items-center ltr:space-x-2 rtl:space-x-reverse rtl:space-x-2">
                            ...
                            <div class="p-4">
                                ...
                                <div v-if="rx.notes" class="mt-2 p-2 bg-yellow-50 rounded text-xs text-gray-600">
                                    <span class="font-medium">{{ $t('a_notes') }}:</span> {{ rx.notes }}
                                </div>
                            </div>
                        </div>
                        ...
                        <div v-if="can('prescriptions.create')">
                            ...
                            <div v-if="showNewPrescription" class="border border-gray-200 rounded-lg p-5 bg-gray-50 mt-2">
                                <h4 class="text-sm font-semibold text-gray-700 mb-4">{{ $t('a_create_prescription_for') }} {{ $localized(doctor, 'name') }}</h4>
                                <form @submit.prevent="submitPrescription" class="space-y-4">
                                    ...
                                    <div>
                                        ...
                                        >
                                            <svg class="w-4 h-4 ltr:mr-1 rtl:ml-1" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4" /></svg>
                                            {{ $t('a_add_medication') }}
                                        </button>
                                    </div>

                                    <div class="flex ltr:space-x-3 rtl:space-x-reverse rtl:space-x-3 pt-2">
                                        <button
                                            type="submit"
                    ...
                    <div v-if="activeTab === 'finance'" class="space-y-6">
                        ...
                        <div v-if="payoutSummary" class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                            <div class="bg-emerald-50 rounded-lg p-4 border border-emerald-100">
                                <p class="text-xs text-emerald-600 uppercase tracking-wider font-semibold">{{ $t('a_total_paid') }}</p>
                                <p class="text-xl font-bold text-emerald-700 mt-1">{{ formatCurrency(payoutSummary.total_paid) }}</p>
                            </div>
                            <div class="bg-amber-50 rounded-lg p-4 border border-amber-100">
                                <p class="text-xs text-amber-600 uppercase tracking-wider font-semibold">{{ $t('a_pending_payment') }}</p>
                                <p class="text-xl font-bold text-amber-700 mt-1">{{ formatCurrency(payoutSummary.total_pending) }}</p>
                            </div>
                            <div class="bg-rose-50 rounded-lg p-4 border border-rose-100">
                                <p class="text-xs text-rose-500 uppercase tracking-wider font-semibold">{{ $t('a_unpaid_commission') }}</p>
                                <p class="text-xl font-bold text-rose-600 mt-1">{{ formatCurrency(payoutSummary.total_unpaid) }}</p>
                            </div>
                        </div>

                        <!-- Recent Payouts + Actions -->
                        <div class="bg-gray-50 rounded-lg p-5">
                            <div class="flex items-center justify-between mb-3">
                                <h3 class="text-sm font-semibold text-gray-700 uppercase tracking-wider">{{ $t('a_recent_payouts') }}</h3>
                                <div class="flex items-center gap-2">
                                    <Link v-if="can('doctor_payouts.create')" :href="`/admin/doctor-payouts/create?doctor_id=${doctor.id}`"
                                        class="text-xs font-medium px-3 py-1.5 rounded-lg text-white transition-colors" style="background-color: #C4A265;">
                                        + Create Payout
                                    </Link>
                                    <Link v-if="can('doctor_payouts.view')" :href="`/admin/doctor-payouts?doctor_id=${doctor.id}`"
                                        class="text-xs font-medium px-3 py-1.5 rounded-lg text-gray-600 bg-white border border-gray-200 hover:bg-gray-100 transition-colors">
                                        View All
                                    </Link>
                                </div>
                            </div>
                            <div v-if="recentPayouts?.length > 0" class="overflow-x-auto">
                                <table class="min-w-full divide-y divide-gray-200">
                                    <thead class="bg-white">
                                        <tr>
                                            <th class="px-4 py-2 ltr:text-left rtl:text-right text-xs font-medium text-gray-500 uppercase">{{ $t('a_payout_number') }}</th>
                                            <th class="px-4 py-2 ltr:text-left rtl:text-right text-xs font-medium text-gray-500 uppercase">{{ $t('a_period') }}</th>
                                            <th class="px-4 py-2 ltr:text-right rtl:text-left text-xs font-medium text-gray-500 uppercase">{{ $t('a_amount') }}</th>
                                            <th class="px-4 py-2 text-center text-xs font-medium text-gray-500 uppercase">{{ $t('a_status') }}</th>
                                            <th class="px-4 py-2"></th>
                                        </tr>
                                    </thead>
                                    <tbody class="divide-y divide-gray-200">
                                        <tr v-for="p in recentPayouts" :key="p.id" class="hover:bg-white/60">
                                            <td class="px-4 py-2 text-xs font-mono font-medium text-gray-700">{{ p.payout_number }}</td>
                                            <td class="px-4 py-2 text-xs text-gray-500">{{ formatDate(p.period_start) }} – {{ formatDate(p.period_end) }}</td>
                                            <td class="px-4 py-2 text-sm font-bold ltr:text-right rtl:text-left" style="color: #C4A265;">{{ formatCurrency(p.net_amount) }}</td>
                                            ...
                                            <td class="px-4 py-2 ltr:text-right rtl:text-left">
                                                <Link v-if="can('doctor_payouts.view')" :href="`/admin/doctor-payouts/${p.id}`"
                                                    class="text-xs font-medium hover:underline" style="color: #C4A265;">{{ $t('a_view') }}</Link>
                                            </td>
                                        </tr>
                                    </tbody>
                                </table>
                            </div>
                            <p v-else class="text-sm text-gray-400 text-center py-4">{{ $t('a_no_payouts_yet') }}</p>
                        </div>

                        <!-- Commission Settings -->
                        <div class="bg-gray-50 rounded-lg p-5">
                            <h3 class="text-sm font-semibold text-gray-700 uppercase tracking-wider mb-3">{{ $t('a_commission_settings') }}</h3>
                            <div class="mb-3 text-sm">
                                <span class="text-gray-500">{{ $t('a_default_commission_rate') }}:</span>
                                <span class="ltr:ml-2 rtl:mr-2 font-bold text-lg" style="color: #C4A265;">{{ doctor.default_commission_percentage || 0 }}%</span>
                            </div>
                            <div v-if="doctor.service_rates && doctor.service_rates.length > 0">
                                <p class="text-xs text-gray-500 uppercase tracking-wider mb-2">{{ $t('a_per_service_rates') }}</p>
                                <table class="min-w-full divide-y divide-gray-200">
                                    <thead class="bg-white">
                                        <tr>
                                            <th class="px-4 py-2 ltr:text-left rtl:text-right text-xs font-medium text-gray-500 uppercase">{{ $t('a_service') }}</th>
                                            <th class="px-4 py-2 ltr:text-right rtl:text-left text-xs font-medium text-gray-500 uppercase">{{ $t('a_commission') }} %</th>
                                        </tr>
                                    </thead>
                                    <tbody class="divide-y divide-gray-200">
                                        <tr v-for="rate in doctor.service_rates" :key="rate.id">
                                            <td class="px-4 py-2 text-sm text-gray-900">{{ $localized(rate.service, 'name') || '-' }}</td>
                                            <td class="px-4 py-2 text-sm font-medium ltr:text-right rtl:text-left" style="color: #C4A265;">{{ rate.commission_percentage }}%</td>
                                        </tr>
                                    </tbody>
                                </table>
                            </div>
                            <p v-else class="text-xs text-gray-400">{{ $t('a_no_custom_rates_default') }}</p>
                        </div>
                        ...
                        <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                            <div class="bg-gray-50 rounded-lg p-5">
                                <h3 class="text-sm font-semibold text-gray-700 uppercase tracking-wider mb-3">{{ $t('a_monthly_commission') }}</h3>
                            ...
                            <div class="bg-gray-50 rounded-lg p-5">
                                <h3 class="text-sm font-semibold text-gray-700 uppercase tracking-wider mb-3">{{ $t('a_monthly_revenue') }}</h3>
                        ...
                        <div>
                            <h3 class="text-sm font-semibold text-gray-700 uppercase tracking-wider mb-3">{{ $t('a_commission_statement') }}</h3>
                                ...
                                <table class="min-w-full divide-y divide-gray-200">
                                    <thead class="bg-gray-50">
                                        <tr>
                                            ...
                                            <th class="px-4 py-3 ltr:text-left rtl:text-right text-xs font-medium text-gray-500 uppercase">{{ $t('a_service') }}</th>
                                            <th class="px-4 py-3 ltr:text-left rtl:text-right text-xs font-medium text-gray-500 uppercase">{{ $t('a_type') }}</th>
                                            <th class="px-4 py-3 ltr:text-right rtl:text-left text-xs font-medium text-gray-500 uppercase">{{ $t('a_invoice_total') }}</th>
                                            <th class="px-4 py-3 ltr:text-right rtl:text-left text-xs font-medium text-gray-500 uppercase">{{ $t('a_supply_cost') }}</th>
                                            <th class="px-4 py-3 ltr:text-right rtl:text-left text-xs font-medium text-gray-500 uppercase">{{ $t('a_rate') }}</th>
                                            <th class="px-4 py-3 ltr:text-right rtl:text-left text-xs font-medium text-gray-500 uppercase">{{ $t('a_commission') }}</th>
                                        </tr>
                                    </thead>
                                    <tbody class="bg-white divide-y divide-gray-200">
                                        <tr v-for="row in commissionStatement" :key="row.id" class="hover:bg-gray-50">
                                            ...
                                            <td class="px-4 py-3 whitespace-nowrap">
                                                <Link v-if="can('patients.view') && row.patient_id" :href="`/admin/patients/${row.patient_id}`" class="text-sm font-medium hover:underline" style="color: #C4A265;">{{ row.patient_name }}</Link>
                                            ...
                                            <td class="px-4 py-3 whitespace-nowrap text-sm text-gray-900 ltr:text-right rtl:text-left">{{ row.invoice_total ? formatCurrency(row.invoice_total) : '-' }}</td>
                                            <td class="px-4 py-3 whitespace-nowrap text-sm text-gray-500 ltr:text-right rtl:text-left">{{ row.supply_cost ? formatCurrency(row.supply_cost) : '-' }}</td>
                                            <td class="px-4 py-3 whitespace-nowrap text-sm font-medium ltr:text-right rtl:text-left" style="color: #C4A265;">{{ row.commission_rate }}%</td>
                                            <td class="px-4 py-3 whitespace-nowrap text-sm font-bold ltr:text-right rtl:text-left text-green-700">{{ formatCurrency(row.commission_amount) }}</td>
                                        </tr>
                                    </tbody>
                                    <tfoot class="bg-gray-50">
                                        <tr>
                                            <td colspan="4" class="px-4 py-3 text-sm font-bold text-gray-700">{{ $t('a_total_showing_last_50') }}</td>
                                            <td class="px-4 py-3 text-sm font-bold text-gray-900 ltr:text-right rtl:text-left">
                                                {{ formatCurrency(commissionStatement.reduce((sum, r) => sum + Number(r.invoice_total || 0), 0)) }}
                                            </td>
                                            <td class="px-4 py-3 text-sm font-bold text-gray-500 ltr:text-right rtl:text-left">
                                                {{ formatCurrency(commissionStatement.reduce((sum, r) => sum + Number(r.supply_cost || 0), 0)) }}
                                            </td>
                                            <td class="px-4 py-3"></td>
                                            <td class="px-4 py-3 text-sm font-bold text-green-700 ltr:text-right rtl:text-left">
                            ...
                            <p v-else class="text-sm text-gray-400 text-center py-8">{{ $t('a_no_commission_records') }}</p>
                        </div>
                    </div>
                    ...
                    <div v-if="activeTab === 'schedule'" class="space-y-6">
                        ...
                        <div v-if="doctor.vacations && doctor.vacations.length > 0">
                            <h3 class="text-sm font-semibold text-gray-700 uppercase tracking-wider mb-3">{{ $t('a_vacations') }}</h3>
                            <div class="space-y-2">
                        ...
                        <!-- Leave Requests -->
                        <div>
                            <h3 class="text-sm font-semibold text-gray-700 uppercase tracking-wider mb-3">{{ $t('a_leave_requests') }}</h3>
                            <div v-if="leaves && leaves.length > 0" class="overflow-x-auto">
                                <table class="min-w-full divide-y divide-gray-200">
                                    <thead class="bg-gray-50">
                                        <tr>
                                            <th class="px-4 py-3 ltr:text-left rtl:text-right text-xs font-medium text-gray-500 uppercase">{{ $t('a_type') }}</th>
                                            <th class="px-4 py-3 ltr:text-left rtl:text-right text-xs font-medium text-gray-500 uppercase">{{ $t('a_from') }}</th>
                                            <th class="px-4 py-3 ltr:text-left rtl:text-right text-xs font-medium text-gray-500 uppercase">{{ $t('a_to') }}</th>
                                            <th class="px-4 py-3 ltr:text-left rtl:text-right text-xs font-medium text-gray-500 uppercase">{{ $t('a_reason') }}</th>
                                            <th class="px-4 py-3 ltr:text-left rtl:text-right text-xs font-medium text-gray-500 uppercase">{{ $t('a_status') }}</th>
                                            <th class="px-4 py-3 ltr:text-left rtl:text-right text-xs font-medium text-gray-500 uppercase">{{ $t('a_approved_by') }}</th>
                                        </tr>
                                    </thead>
                                    <tbody class="bg-white divide-y divide-gray-200">
                            ...
                            <p v-else class="text-sm text-gray-400 text-center py-6">{{ $t('a_no_leave_requests') }}</p>
                        </div>
                    </div>
                        ...
                        <div class="overflow-x-auto">
                            <table class="min-w-full divide-y divide-gray-200">
                                <thead class="bg-gray-50">
                                    <tr>
                                        <th class="px-4 py-3 ltr:text-left rtl:text-right text-xs font-medium text-gray-500 uppercase">{{ $t('a_date') }}</th>
                                        <th class="px-4 py-3 ltr:text-left rtl:text-right text-xs font-medium text-gray-500 uppercase">{{ $t('a_client') }}</th>
                                        <th class="px-4 py-3 ltr:text-left rtl:text-right text-xs font-medium text-gray-500 uppercase">{{ $t('a_phone') }}</th>
                                        <th class="px-4 py-3 ltr:text-left rtl:text-right text-xs font-medium text-gray-500 uppercase">{{ $t('a_service') }}</th>
                                        <th class="px-4 py-3 ltr:text-left rtl:text-right text-xs font-medium text-gray-500 uppercase">{{ $t('a_time') }}</th>
                                        <th class="px-4 py-3 ltr:text-left rtl:text-right text-xs font-medium text-gray-500 uppercase">{{ $t('a_status') }}</th>
                                        <th class="px-4 py-3 ltr:text-right rtl:text-left text-xs font-medium text-gray-500 uppercase">{{ $t('a_actions') }}</th>
                                    </tr>
                                </thead>
                                <tbody class="bg-white divide-y divide-gray-200">
                                    <tr v-for="booking in bookings" :key="booking.id" class="hover:bg-gray-50">
                                        ...
                                        <td class="px-4 py-3 whitespace-nowrap text-sm text-gray-500">{{ booking.service ? $localized(booking.service, 'name') : '-' }}</td>
                                        ...
                                        <td class="px-4 py-3 whitespace-nowrap ltr:text-right rtl:text-left">
                                            <Link v-if="can('bookings.view')" :href="`/admin/bookings/${booking.id}`" class="text-sm font-medium hover:underline" style="color: #C4A265;">{{ $t('a_view') }}</Link>
                                        </td>
                                    </tr>
                                    <tr v-if="!bookings || bookings.length === 0">
                                        <td colspan="7" class="px-4 py-8 text-center text-sm text-gray-500">{{ $t('a_no_bookings_found_dot') }}</td>
                                    </tr>
                                </tbody>
                            </table>
                        </div>
