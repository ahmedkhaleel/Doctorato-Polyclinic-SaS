...
function submit() {
    form.post(lp('/forgot-password'), { preserveScroll: true });
}
</script>
        ...
        <div class="w-full max-w-md">
            ...
            <div class="bg-white/5 backdrop-blur-md rounded-2xl border border-white/10 shadow-2xl p-7">
                <h1 class="text-white text-xl font-bold mb-2">
                    {{ isRtl ? 'استعادة كلمة المرور' : 'Forgot your password?' }}
                </h1>
                ...
                <div v-if="flashSuccess" class="mb-4 p-3 rounded-lg bg-emerald-500/10 border border-emerald-500/30 text-emerald-300 text-sm">
                    {{ flashSuccess }}
                </div>
                ...
                <div class="mt-6 text-center">
                    <Link :href="lp('/login')" class="text-sm text-gray-400 hover:text-[#C4A265] transition">
