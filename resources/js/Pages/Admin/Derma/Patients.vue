...
<template>
    <div class="space-y-6 pb-10">
        <div class="bg-gradient-to-br from-pink-600 to-rose-500 rounded-2xl p-6 shadow-lg">
            ...
            <p class="text-pink-100/80 text-sm mt-1">{{ t('All patients with dermatology visits', 'جميع المرضى الذين لديهم زيارات جلدية') }}</p>
        </div>

        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-4">
            <input v-model="search" type="text"
                :placeholder="t('Search by name, phone, file #', 'بحث بالاسم أو الهاتف أو رقم الملف')"
                class="w-full md:w-96 px-4 py-2.5 border border-gray-200 rounded-xl text-sm focus:ring-2 focus:ring-pink-500/20 focus:border-pink-400" />
        </div>
                ...
                <tbody>
                    <tr v-for="p in patients.data" :key="p.id" class="border-t border-gray-100 hover:bg-gray-50">
                        ...
                        <td class="px-5 py-3 text-end">
                            <Link :href="`/admin/patients/${p.id}?tab=derma`" class="text-pink-600 text-xs font-semibold">{{ t('Open', 'فتح') }}</Link>
                        </td>
                    </tr>
