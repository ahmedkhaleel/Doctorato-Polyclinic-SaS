...
function generateSlug() {
    form.name = form.display_name_en
        .toLowerCase().replace(/[^a-z0-9\s]/g, '').replace(/\s+/g, '_').replace(/_+/g, '_').replace(/^_|_$/g, '');
}
                    ...
                    <div class="p-6">
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-5">
                            <div>
                                ...
                                <input v-model="form.display_name_en" @blur="generateSlug" type="text" placeholder="e.g. Content Editor"
                            ...
                            <div>
                                ...
                                <input v-model="form.display_name_ar" type="text" dir="rtl" placeholder="مثال: محرر المحتوى"
                                    class="doctorato-input w-full px-4 py-2.5 border border-gray-200 rounded-xl text-sm bg-gray-50/50 hover:border-gray-300 focus:ring-2 focus:ring-[#C4A265]/20 focus:border-[#C4A265] outline-none transition" />
                            ...
                            <div>
                                ...
                                <input v-model="form.name" type="text" placeholder="auto_generated_slug"
                                    class="doctorato-input w-full px-4 py-2.5 border border-gray-200 rounded-xl text-sm font-mono bg-gray-100/70 hover:border-gray-300 focus:ring-2 focus:ring-[#C4A265]/20 focus:border-[#C4A265] outline-none transition" />
                    ...
                    <div class="flex items-center gap-4">
                        ...
                        <div class="hidden sm:block w-48 h-2 bg-gray-100 rounded-full overflow-hidden">
                            <div class="h-full rounded-full transition-all duration-500 ease-out" style="background: linear-gradient(90deg, #C4A265, #a8884f);"
                                :style="{ width: `${totalPerms ? (form.permissions.length / totalPerms * 100) : 0}%` }"></div>
                        </div>
                    </div>
                                                ...
                                                <td v-for="action in getGroupActions(groupKey)" :key="action" class="px-2 py-3 text-center">
                                                    <template v-if="mod.actions.includes(action)">
                                                        <label class="relative inline-flex cursor-pointer">
                                                            <input type="checkbox" :checked="isChecked(mod.key, action)" @change="togglePermission(mod.key, action)" class="sr-only peer" />
                                                            <div class="w-5 h-5 rounded-md border-2 transition-all duration-200 flex items-center justify-center
                                                                ...
                                                                :class="isChecked(mod.key, action) ? '' : 'border-gray-300 hover:border-gray-400 bg-white'"
