        ...
        <div class="receipt-container mx-auto max-w-[800px] bg-white rounded-2xl shadow-sm border border-gray-100/80 print:shadow-none print:border-0 print:rounded-none">
            <div class="p-8 print:p-0">
                ...
                <div class="grid grid-cols-2 gap-6 mb-8">
                    <div class="space-y-2">
                        <h3 class="text-xs font-semibold text-gray-400 uppercase tracking-wider mb-3">{{ $t('a_booking_details') }}</h3>
                    ...
                    <div class="space-y-2 text-end">
                        ...
                        <div v-if="booking.patient?.file_number" class="flex items-baseline gap-2 justify-end">
                ...
                <div class="mb-8">
                    ...
                    <table class="w-full text-sm border border-gray-200">
                        <thead>
                            <tr class="bg-gray-50">
                                <th class="px-4 py-2.5 text-start text-xs font-semibold text-gray-600 border-b border-gray-200">
                                    {{ $t('a_service') }}
                                </th>
                                <th class="px-4 py-2.5 text-center text-xs font-semibold text-gray-600 border-b border-gray-200">
                                    {{ $t('a_sessions') }}
                                </th>
                                <th class="px-4 py-2.5 text-end text-xs font-semibold text-gray-600 border-b border-gray-200">
                                    {{ $t('a_unit_price') }}
                                </th>
                                <th class="px-4 py-2.5 text-end text-xs font-semibold text-gray-600 border-b border-gray-200">
                                    {{ $t('a_total') }}
                        ...
                        <tfoot>
                            <tr class="bg-gray-50 border-t-2 border-gray-300">
                                <td colspan="3" class="px-4 py-3 text-end text-sm font-bold text-gray-700">
                                    {{ $t('a_grand_total') }}
                                </td>
                                <td class="px-4 py-3 text-end text-sm font-bold" style="color: #C4A265;">
                ...
                <!-- Payment Details -->
                <div class="mb-8">
                    <h3 class="text-xs font-semibold text-gray-400 uppercase tracking-wider mb-3">{{ $t('a_payment_details') }}</h3>
                        ...
                        <table v-if="booking.invoice?.payments && booking.invoice.payments.length > 0" class="w-full text-sm">
                            <thead>
                                <tr class="bg-gray-50">
                                    ...
                                    <th class="px-4 py-2.5 text-start text-xs font-semibold text-gray-600 border-b border-gray-200">{{ $t('a_received_by') }}</th>
                                </tr>
                            </thead>
                            <tbody class="divide-y divide-gray-100">
                                <tr v-for="payment in booking.invoice.payments" :key="payment.id">
                                    ...
                                    <td class="px-4 py-2.5 text-end font-semibold text-emerald-600">{{ formatCurrency(payment.amount) }}</td>
                        ...
                        <div class="bg-gray-50 border-t border-gray-200 px-4 py-4">
                            <div class="grid grid-cols-2 gap-3 max-w-md ltr:ml-auto rtl:mr-auto">
                                <div class="flex justify-between col-span-2 text-sm">
                                    <span class="text-gray-500">{{ $t('a_subtotal') }}</span>
                                ...
                                <div class="flex justify-between col-span-2 text-sm border-t border-gray-200 pt-2">
                                    <span class="font-bold text-gray-700">{{ $t('a_total') }}</span>
                                    <span class="font-bold text-gray-800">{{ formatCurrency(booking.invoice?.total) }}</span>
                                </div>
                                <div class="flex justify-between col-span-2 text-sm">
                                    <span class="font-bold text-gray-700">{{ $t('a_amount_paid') }}</span>
