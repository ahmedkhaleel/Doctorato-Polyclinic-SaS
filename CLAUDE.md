...
| `php artisan db:seed --class=Database\\Seeders\\MonthlyAppointmentsDemoSeeder` | On-demand, demo/staging only. Fills the **current month** with appointments for **every active doctor** in three states: past = completed visits (+invoice/payment/commission), today = in_progress + waiting (chair queue), future = confirmed upcoming (no visit yet). Idempotent (bookings tagged `source='monthly_demo'`; skips a doctor whose month is already filled). NOT in the auto-deploy path. |
| `php artisan db:seed --class=Database\\Seeders\\FullDemoSeeder` | On-demand, demo/staging only. **One-stop navigable demo:** enables all medical modules (+ telemedicine) then runs `SpecialtyDoctorDemoSeeder` + `DemoUserSeeder` — a login for every ROLE (`demo.admin`/`demo.secretary`/`demo.patient`/`demo.doctor`) AND every SPECIALTY (`demo.<module>`), all over shared, fully-populated clinic data so secretary front-desks / admin dashboards / patient portal all show real content. Idempotent. NOT in the auto-deploy path. |
