    ...
    <AdminLayout :title="$t('a_new_leave_request')">
        <div class="space-y-6">
            <div class="flex items-center justify-between">
                <h1 class="text-2xl font-bold text-gray-800">{{ $t('a_new_leave_request') }}</h1>
            ...
            <form @submit.prevent="submit" class="max-w-2xl">
                <div class="bg-white rounded-lg shadow-sm p-6 space-y-4">
                    ...
                    <div>
                        ...
                        <select v-model="form.leave_type" class="w-full px-4 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-yellow-200 focus:border-transparent">
                    ...
                    <div>
                        ...
                        <textarea v-model="form.reason" rows="4" class="w-full px-4 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-yellow-200 focus:border-transparent" :placeholder="$t('a_leave_reason_placeholder')"></textarea>
                ...
                <div :class="['flex mt-6 space-x-3 rtl:space-x-reverse', isRtl ? 'justify-start' : 'justify-end']">
                    <Link href="/admin/leaves" class="px-6 py-3 rounded-lg border border-gray-300 text-gray-600 font-medium text-sm hover:bg-gray-50 transition">{{ $t('a_cancel') }}</Link>
                    <button
                        type="submit"
