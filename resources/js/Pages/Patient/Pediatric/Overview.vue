    ...
    <div class="max-w-5xl mx-auto px-4 py-8 space-y-6">
        ...
        <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
            <div class="bg-white rounded-2xl border border-gray-100 p-4 text-center">
                <p class="text-2xl font-bold text-emerald-600">{{ visitSummary?.total ?? 0 }}</p>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <a href="/patient/pediatric/vaccination-card" target="_blank" class="bg-white rounded-2xl border border-gray-100 p-5 hover:shadow-md transition-shadow group flex items-center gap-3">
                <div class="w-10 h-10 rounded-lg bg-slate-50 flex items-center justify-center group-hover:bg-slate-100 transition-colors">
                    <svg class="w-5 h-5 text-[#1B365D]" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M12 10v6m0 0l-3-3m3 3l3-3m2 8H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/></svg>
                </div>
                <div>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
            ...
            <Link href="/patient/pediatric/growth" class="bg-white rounded-2xl border border-gray-100 p-5 hover:shadow-md transition-shadow group">
                <div class="flex items-center gap-3">
                    <div class="w-10 h-10 rounded-lg bg-slate-50 flex items-center justify-center group-hover:bg-slate-100 transition-colors">
                        <svg class="w-5 h-5 text-[#1B365D]" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M13 7h8m0 0v8m0-8l-8 8-4-4-6 6" /></svg>
                    </div>
                    <div>
            ...
            <Link href="/patient/pediatric/visits" class="bg-white rounded-2xl border border-gray-100 p-5 hover:shadow-md transition-shadow group">
                <div class="flex items-center gap-3">
                    <div class="w-10 h-10 rounded-lg bg-slate-50 flex items-center justify-center group-hover:bg-slate-100 transition-colors">
                        <svg class="w-5 h-5 text-[#1B365D]" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2" /></svg>
                    </div>
                    <div>
        ...
        <div v-if="allergies?.length || chronicConditions?.length" class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div v-if="allergies?.length" class="bg-white rounded-2xl border border-gray-100 overflow-hidden">
                ...
                <div class="p-5 space-y-2">
                    <div v-for="a in allergies" :key="a.id" class="flex items-center gap-2">
                        <span class="w-2 h-2 rounded-full" :class="a.severity === 'severe' ? 'bg-red-500' : a.severity === 'moderate' ? 'bg-amber-500' : 'bg-emerald-500'"></span>
            ...
            <div v-if="chronicConditions?.length" class="bg-white rounded-2xl border border-gray-100 overflow-hidden">
                <div class="px-5 py-4 border-b border-gray-100">
                    <h2 class="text-sm font-bold text-[#1B365D]">{{ isRtl ? 'الأمراض المزمنة' : 'Chronic Conditions' }}</h2>
                </div>
                <div class="p-5 space-y-2">
