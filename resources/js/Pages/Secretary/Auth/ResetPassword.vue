...
function submit() {
    form.post('/secretary/reset-password', {
        preserveScroll: true,
    ...
    <div :dir="isRtl ? 'rtl' : 'ltr'"
        ...
        <div class="w-full max-w-md">
            <div class="text-center mb-8">
                ...
                <p class="text-gray-400 text-sm uppercase tracking-widest">{{ isRtl ? 'بوابة السكرتارية' : 'Secretary Portal' }}</p>
            </div>

            <div class="bg-white/5 backdrop-blur-md rounded-2xl border border-white/10 shadow-2xl p-7">
                <h1 class="text-white text-xl font-bold mb-6">{{ isRtl ? 'تعيين كلمة مرور جديدة' : 'Set a new password' }}</h1>
                ...
                <form @submit.prevent="submit" class="space-y-4">
                    ...
                    <div>
                        <label class="block text-xs font-semibold text-gray-300 mb-1.5">{{ isRtl ? 'كلمة المرور الجديدة' : 'New password' }}</label>
                        <input v-model="form.password" type="password" required minlength="8" autocomplete="new-password"
                            class="w-full px-4 py-2.5 rounded-xl bg-white/5 border border-white/10 text-white focus:border-[#C4A265] focus:ring-2 focus:ring-[#C4A265]/30 outline-none transition" />
                        <p v-if="form.errors.password" class="text-red-400 text-xs mt-1">{{ form.errors.password }}</p>
                    </div>
                    <div>
                        <label class="block text-xs font-semibold text-gray-300 mb-1.5">{{ isRtl ? 'تأكيد كلمة المرور' : 'Confirm password' }}</label>
                    ...
                    <button type="submit" :disabled="form.processing"
                        ...
                        {{ form.processing ? (isRtl ? 'جارٍ الحفظ...' : 'Saving...') : (isRtl ? 'حفظ كلمة المرور' : 'Reset Password') }}
                    </button>
                </form>
            </div>
        </div>
