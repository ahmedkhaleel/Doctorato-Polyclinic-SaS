    ...
    <AdminLayout :title="$t('a_shifts')">
        <div class="space-y-6">
            <div class="flex items-center justify-between">
                <h1 class="text-xl md:text-2xl font-bold text-gray-800">{{ $t('a_shifts') }}</h1>
            </div>
                ...
                <div class="overflow-x-auto">
                    <table class="min-w-full divide-y divide-gray-200">
                        <thead class="bg-gray-50">
                            <tr>
                                <th class="px-4 md:px-6 py-3 ltr:text-left rtl:text-right text-xs font-medium text-gray-500 uppercase tracking-wider">{{ $t('a_name_ar') }}</th>
                                <th class="px-4 md:px-6 py-3 ltr:text-left rtl:text-right text-xs font-medium text-gray-500 uppercase tracking-wider">{{ $t('a_name_en') }}</th>
                                <th class="px-4 md:px-6 py-3 ltr:text-left rtl:text-right text-xs font-medium text-gray-500 uppercase tracking-wider">{{ $t('a_start_time') }}</th>
                                <th class="px-4 md:px-6 py-3 ltr:text-left rtl:text-right text-xs font-medium text-gray-500 uppercase tracking-wider">{{ $t('a_end_time') }}</th>
                                <th class="px-4 md:px-6 py-3 ltr:text-left rtl:text-right text-xs font-medium text-gray-500 uppercase tracking-wider">{{ $t('a_active') }}</th>
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            <template v-for="shift in shifts" :key="shift.id">
                                ...
                                <tr v-if="editingId !== shift.id" class="hover:bg-gray-50">
                                    <td class="px-4 md:px-6 py-4 whitespace-nowrap text-sm text-gray-900" dir="rtl">{{ shift.name_ar }}</td>
                                    <td class="px-4 md:px-6 py-4 whitespace-nowrap text-sm text-gray-900">{{ shift.name_en }}</td>
                                    <td class="px-4 md:px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ shift.start_time }}</td>
                                    <td class="px-4 md:px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ shift.end_time }}</td>
                                    <td class="px-4 md:px-6 py-4 whitespace-nowrap">
                                        <span :class="shift.is_active ? 'bg-emerald-100 text-emerald-800' : 'bg-gray-100 text-gray-800'" class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full">
                                ...
                                <tr v-else class="bg-amber-50">
                                    ...
                                    <td class="px-4 md:px-6 py-3 ltr:text-right rtl:text-left space-x-2 rtl:space-x-reverse">
                            ...
                            <tr v-if="!shifts || shifts.length === 0">
                                <td colspan="6" class="px-4 md:px-6 py-8 text-center text-sm text-gray-500">{{ $t('a_no_shifts_found') }}</td>
                            </tr>
                        </tbody>
                    </table>
                </div>
