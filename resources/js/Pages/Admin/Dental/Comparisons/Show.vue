            ...
            <div class="dental-hero-enter relative overflow-hidden rounded-2xl bg-gradient-to-br from-[#1B365D] via-[#1B365D] to-[#0F2444] p-6 sm:p-7">
                ...
                <div class="relative z-10 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                        ...
                        <div>
                            <div class="flex items-center gap-2 mb-1">
                                <span :class="'bg-gradient-to-r ' + (categoryColors[comp.category] || 'from-gray-500 to-gray-600')"
                                    class="text-white text-[10px] font-bold px-2.5 py-0.5 rounded-full">
                                    {{ categoryLabel(comp.category) }}
                                </span>
                                <span v-if="comp.is_featured" class="bg-amber-400/20 text-amber-200 text-[10px] font-medium px-2 py-0.5 rounded-full flex items-center gap-1">
                    ...
                    <div class="flex items-center gap-2">
                        ...
                        <button @click="confirmDelete" class="w-10 h-10 rounded-xl bg-red-500/10 backdrop-blur-sm flex items-center justify-center hover:bg-red-500/20 transition ring-1 ring-red-400/20" :aria-label="isRtl ? 'حذف' : 'Delete'" :title="isRtl ? 'حذف' : 'Delete'">
                            <svg class="w-5 h-5 text-red-300" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" /></svg>
                        </button>
                    </div>
                </div>
            </div>
