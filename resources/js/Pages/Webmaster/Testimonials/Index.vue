...
watch(search, (val) => {
    ...
    searchTimeout = setTimeout(() => {
        router.get('/webmaster/testimonials', { search: val || undefined }, {
            preserveState: true,
            replace: true,
        });
    }, 400);
...
function deleteTestimonial(id) {
    confirm(isRtl.value ? 'هل أنت متأكد من حذف هذه الشهادة؟' : 'Are you sure you want to delete this testimonial?', () => {
                ...
                <div class="overflow-x-auto">
                    <table class="min-w-full divide-y divide-gray-200">
                        <thead class="bg-gray-50">
                            <tr>
                                <th class="px-6 py-3 ltr:text-left rtl:text-right text-xs font-medium text-gray-500 uppercase tracking-wider">{{ $t('a_client') }}</th>
                                <th class="px-6 py-3 ltr:text-left rtl:text-right text-xs font-medium text-gray-500 uppercase tracking-wider">{{ $t('a_rating') }}</th>
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            <tr v-for="t in testimonials.data" :key="t.id" class="hover:bg-gray-50">
                                <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">{{ t.client_name_en }}</td>
                                ...
                                <td class="px-6 py-4 whitespace-nowrap ltr:text-right rtl:text-left text-sm ltr:space-x-3 rtl:space-x-reverse rtl:space-x-3">
                                    <Link v-if="can('testimonials.update')" :href="`/webmaster/testimonials/${t.id}/edit`" class="font-medium hover:underline" style="color: #C4A265;">{{ $t('a_edit') }}</Link>
