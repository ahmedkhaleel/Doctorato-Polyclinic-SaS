...
function doCheckOut() {
    ...
    getLocation((lat, lng) => {
        locationStatus.value = isRtl.value ? 'جاري التسجيل...' : 'Recording...';
        router.post('/secretary/my-attendance/check-out', { lat, lng }, {
            preserveScroll: true,
        ...
        locationStatus.value = isRtl.value ? 'جاري التسجيل بدون موقع...' : 'Recording without location...';
        router.post('/secretary/my-attendance/check-out', { lat: null, lng: null }, {
            preserveScroll: true,
            onFinish: () => { checkingOut.value = false; locationStatus.value = ''; },
        });
    });
...
function applyFilter() {
    router.get('/secretary/my-attendance', { month: selectedMonth.value, year: selectedYear.value }, { preserveState: true, replace: true });
}
...
<template>
    <div>
            ...
            <div class="relative z-10">
                <div class="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-4">
                    <div>
                        ...
                        <p class="text-sm text-gray-400 mt-1.5">{{ isRtl ? 'سجّل حضورك وانصرافك بضغطة زر' : 'Check in and out with one tap' }}</p>
                    </div>
                    <!-- Filter -->
                ...
                <div class="grid grid-cols-2 sm:grid-cols-5 gap-3 mt-6">
                    <div class="bg-white/5 backdrop-blur-sm rounded-xl p-3.5 border border-white/10">
                        <p class="text-xs text-gray-400 font-medium">{{ isRtl ? 'حاضر' : 'Present' }}</p>
                        <p class="text-2xl font-bold text-emerald-400 mt-1">{{ summary?.present ?? 0 }}</p>
                    </div>
                    <div class="bg-white/5 backdrop-blur-sm rounded-xl p-3.5 border border-white/10">
                        <p class="text-xs text-gray-400 font-medium">{{ isRtl ? 'غائب' : 'Absent' }}</p>
                        <p class="text-2xl font-bold text-red-400 mt-1">{{ summary?.absent ?? 0 }}</p>
                    </div>
                    <div class="bg-white/5 backdrop-blur-sm rounded-xl p-3.5 border border-white/10">
                        <p class="text-xs text-gray-400 font-medium">{{ isRtl ? 'متأخر' : 'Late' }}</p>
                        <p class="text-2xl font-bold text-amber-400 mt-1">{{ summary?.late ?? 0 }}</p>
                    </div>
                    <div class="bg-white/5 backdrop-blur-sm rounded-xl p-3.5 border border-white/10">
        ...
        <div
            class="bg-white rounded-2xl border border-gray-100 shadow-lg overflow-hidden mb-6 transition-all duration-700"
            ...
            <div class="p-4 sm:p-6">
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    ...
                    <div class="relative">
                        ...
                        >
                            <!-- Pulse ring animation when not checked in -->
                            ...
                            <p v-if="hasCheckedIn" class="text-sm font-semibold text-emerald-600 mt-1" dir="ltr">
                    ...
                    <div class="relative">
                        ...
                        >
                            ...
                            <p v-if="hasCheckedOut" class="text-sm font-semibold text-red-600 mt-1" dir="ltr">
                ...
                <p v-if="$page.props.flash?.error" class="mt-3 text-xs text-red-600 font-semibold text-center bg-red-50 rounded-lg py-2.5">
        ...
        <!-- ═══ ATTENDANCE RECORDS ═══ -->
        <div class="space-y-2 transition-all duration-500" :class="cardsLoaded ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'">
            ...
            <div
                ...
                :key="record.id"
                class="bg-white rounded-xl shadow-sm border border-gray-100/80 hover:shadow-md transition-all duration-300 p-4"
            >
                <div class="flex items-center gap-4">
                    <!-- Date -->
                    <div :class="[getStatus(record.status).bg, getStatus(record.status).border]" class="w-12 h-12 rounded-xl border flex flex-col items-center justify-center flex-shrink-0">
                        <span :class="getStatus(record.status).text" class="text-sm font-bold leading-none">{{ record.date ? new Date(record.date).getDate() : '-' }}</span>
                        <span :class="getStatus(record.status).text" class="text-[9px] font-medium mt-0.5 opacity-70">
                    ...
                    <!-- Status -->
                    <span :class="[getStatus(record.status).bg, getStatus(record.status).text]" class="inline-flex items-center gap-1.5 text-[11px] font-semibold px-3 py-1.5 rounded-full flex-shrink-0">
                        <span class="w-1.5 h-1.5 rounded-full" :class="getStatus(record.status).dot"></span>
        ...
        <!-- Empty -->
        <div v-if="!records || records.length === 0" class="py-16 text-center bg-white rounded-xl border border-gray-100 mt-4">
