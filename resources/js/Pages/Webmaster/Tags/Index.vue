...
watch(search, (val) => {
    ...
    searchTimeout = setTimeout(() => {
        router.get('/webmaster/tags', { search: val || undefined }, {
            preserveState: true,
            replace: true,
        });
    }, 400);
...
<template>
    <WebmasterLayout title="Tags">
        <div class="space-y-6">
            <div class="flex items-center justify-between">
                <h1 class="text-2xl font-bold text-gray-800">{{ $t('a_tags') }}</h1>
                <Link
                    v-if="can('tags.create')"
                    href="/webmaster/tags/create"
                    ...
                    <table class="min-w-full divide-y divide-gray-200">
                        <thead class="bg-gray-50">
                            <tr>
                                ...
                                <th class="px-6 py-3 ltr:text-left rtl:text-right text-xs font-medium text-gray-500 uppercase tracking-wider">{{ $t('a_posts_count') }}</th>
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            <tr v-for="tag in tags.data" :key="tag.id" class="hover:bg-gray-50">
                                ...
                                <td class="px-6 py-4 text-sm font-medium text-gray-900">{{ tag.name_en }}</td>
                                <td class="px-6 py-4 text-sm text-gray-500" dir="rtl">{{ tag.name_ar }}</td>
                                ...
                                <td class="px-6 py-4 whitespace-nowrap ltr:text-right rtl:text-left text-sm ltr:space-x-3 rtl:space-x-reverse rtl:space-x-3">
                                    <Link v-if="can('tags.update')" :href="`/webmaster/tags/${tag.id}/edit`" class="font-medium hover:underline" style="color: #C4A265;">{{ $t('a_edit') }}</Link>
                            ...
                            <tr v-if="!tags.data || tags.data.length === 0">
                                <td colspan="5" class="px-6 py-8 text-center text-sm text-gray-500">{{ $t('a_no_tags_found') }}</td>
                            </tr>
                        </tbody>
                    </table>
                </div>

                <div v-if="tags.links && tags.links.length > 3" class="px-6 py-3 border-t border-gray-200 flex items-center justify-between">
