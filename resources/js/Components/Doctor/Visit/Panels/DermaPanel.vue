    ...
    >
        ...
        >
            <div class="flex items-center justify-between">
                ...
                <Link :href="`/doctor/derma/patients/${visit.patient_id}`"
        ...
        <div class="p-4 sm:p-6 space-y-6">
            ...
            <template v-else>
                ...
                <div class="flex flex-wrap gap-2 pt-2 border-t border-gray-100">
                    <Link :href="`/doctor/derma/patients/${visit.patient_id}`" class="inline-flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium rounded-lg border transition-all" :style="`color:${ACCENT}; background:${ACCENT}0D; border-color:${ACCENT}33`">
                        <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" /></svg>
                        {{ isRtl ? 'ملف المريض' : 'Patient Record' }}
                    </Link>
                    <Link :href="`/doctor/derma/treatment-plans`" class="inline-flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium text-[#1B365D] bg-slate-50 hover:bg-slate-100 rounded-lg border border-slate-200 transition-all">
