    ...
    <AdminLayout :title="$t('a_messages')">
        <div class="space-y-6">
            <div class="flex items-center justify-between">
                <h1 class="text-2xl font-bold text-gray-800">{{ $t('a_messages') }}</h1>
            </div>
                ...
                <div class="overflow-x-auto">
                    <table class="min-w-full divide-y divide-gray-200">
                        <thead class="bg-gray-50">
                            <tr>
                                <th class="px-6 py-3 ltr:text-left rtl:text-right text-xs font-medium text-gray-500 uppercase tracking-wider">ID</th>
                                ...
                                <th class="px-6 py-3 ltr:text-left rtl:text-right text-xs font-medium text-gray-500 uppercase tracking-wider">{{ $t('a_subject') }}</th>
                                <th class="px-6 py-3 ltr:text-left rtl:text-right text-xs font-medium text-gray-500 uppercase tracking-wider">{{ $t('a_status') }}</th>
                                <th class="px-6 py-3 ltr:text-left rtl:text-right text-xs font-medium text-gray-500 uppercase tracking-wider">{{ $t('a_created_at') }}</th>
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            <tr
                                ...
                                :class="{ 'bg-yellow-50': !msg.is_read }"
                            >
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ msg.id }}</td>
                                <td class="px-6 py-4 text-sm text-gray-900" :class="{ 'font-bold': !msg.is_read }">{{ msg.name }}</td>
                                <td class="px-6 py-4 text-sm text-gray-500">{{ msg.email }}</td>
                                <td class="px-6 py-4 text-sm text-gray-500 max-w-xs truncate">{{ msg.subject || '-' }}</td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm">
                                    <span
                                        class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium"
                                        :class="msg.is_read ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'"
                                    >
                                        {{ msg.is_read ? $t('a_read') : $t('a_unread') }}
                                    </span>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ formatDate(msg.created_at) }}</td>
                            ...
                            <tr v-if="!messages.data || messages.data.length === 0">
                                <td colspan="7" class="px-6 py-8 text-center text-sm text-gray-500">{{ $t('a_no_messages_found') }}</td>
                            </tr>
                        </tbody>
                    </table>
                </div>

                <div v-if="messages.links && messages.links.length > 3" class="px-6 py-3 border-t border-gray-200 flex items-center justify-between">
