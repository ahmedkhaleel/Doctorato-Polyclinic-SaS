...
const form = useForm({
    ...
    contact_person: '', address: '', notes: '', is_active: true, logo: null,
});

function submit() {
    form.post('/admin/insurance/companies', { forceFormData: true });
}
</script>

<template>
    <AdminLayout :title="t('إضافة شركة تأمين', 'Add Insurance Company')">
        <div class="max-w-3xl mx-auto p-4 md:p-6 space-y-5" :dir="isRtl ? 'rtl' : 'ltr'">
            <div class="flex items-center justify-between">
                <h1 class="text-xl font-bold text-[#1B365D]">{{ t('إضافة شركة تأمين', 'Add Insurance Company') }}</h1>
            ...
            <form @submit.prevent="submit" class="bg-white rounded-2xl border border-gray-100 shadow-sm p-5 space-y-4">
                <div class="grid sm:grid-cols-2 gap-4">
                    ...
                    <label class="block sm:col-span-2"><span class="text-xs font-medium text-gray-500">{{ t('الشعار', 'Logo') }}</span>
                        <input type="file" accept="image/*" @input="form.logo = $event.target.files[0]" class="mt-1 w-full text-sm" />
                        <span v-if="form.errors.logo" class="text-xs text-red-500">{{ form.errors.logo }}</span></label>
                </div>
                ...
                <div class="flex justify-end gap-2 pt-2">
                    ...
                    <button type="submit" :disabled="form.processing || !form.name_ar || !form.name_en" class="px-5 py-2 rounded-lg text-white text-sm font-semibold disabled:opacity-50" style="background:#1B365D">{{ t('حفظ', 'Save') }}</button>
                </div>
            </form>
        </div>
    </AdminLayout>
